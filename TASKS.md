...
- The current root-cause repair is committed in `72d0669…` and is bounded provider-facing data normalization, not another routing alias: keep internal Vault reads at 256 KiB, cap provider-facing reads at 24 KiB with valid UTF-8 truncation, and apply the same cap to inspection, continuation and assisted-edit tool callbacks.
...
- The repair is committed and pushed as an intermediate candidate, but the branch is being synchronized with current canonical `e0fa5cc…`; final exact-head QA/CI and candidate identity are still pending. No credential value was read/printed, no direct Production DB mutation occurred, and the deployed `06a7277…` release must remain the rollback baseline.

## Next unfinished checkpoint

Finish the canonical sync merge, verify the resulting exact SHA through CI and the read-only source/revision/lease/DB16/backup checks plus typed M16 dry-run, then request one new exact-SHA Production approval. PR #57 is already merged at the prior deployed SHA, so any review requirement for this candidate must use the current repository review state. After approval, deploy only that SHA and rerun enrollment/restart, worker reuse, Auto-Chain lineage and the remaining UAT/15-of-15 gates.
