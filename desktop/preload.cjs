...
contextBridge.exposeInMainWorld('artAgent', Object.freeze({
  ...
  loginDevice: (username, password) => ipcRenderer.invoke(CHANNELS.enrollmentLogin, typeof username === 'string' ? username.slice(0, 64) : '', typeof password === 'string' ? password.slice(0, 512) : ''),
  pairDevice: (pairingCode) => ipcRenderer.invoke(CHANNELS.enrollmentPair, typeof pairingCode === 'string' ? pairingCode.slice(0, 128) : ''),
  createDeviceCode: () => ipcRenderer.invoke(CHANNELS.enrollmentIssuePairing),
  ...
  openOwnerRecovery: () => ipcRenderer.invoke(CHANNELS.ownerPasswordReset),
  ...
  getWorkConversation: () => ipcRenderer.invoke(CHANNELS.workConversation),
  submitWorkMessage: (message, idempotencyKey) => ipcRenderer.invoke(CHANNELS.workSubmit, typeof message === 'string' ? message.slice(0, 2_000) : '', typeof idempotencyKey === 'string' ? idempotencyKey.slice(0, 120) : ''),
  getWorkspaceContinuity: () => ipcRenderer.invoke(CHANNELS.workspaceContinuity),
  syncWorkspaceForHandoff: () => ipcRenderer.invoke(CHANNELS.workspaceSync),
  takeOverWorkspace: () => ipcRenderer.invoke(CHANNELS.workspaceTakeover),
