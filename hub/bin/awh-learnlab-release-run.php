...
if(!is_string($execution)||preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i',$execution)!==1){
    fwrite(STDERR,"LEARNLAB_RELEASE_EXECUTION_INVALID\n");exit(2);
}
try{
    ...
    $result=HubLearnLabReleaseOperator::fromEnvironment($pdo)->runExecution(strtolower($execution));
    fwrite(STDOUT,json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_THROW_ON_ERROR)."\n");
}catch(HubLearnLabReleaseOperatorException|HubLearnLabReleaseException $error){
    fwrite(STDERR,$error->codeName."\n");exit(1);
}catch(Throwable){
    fwrite(STDERR,"LEARNLAB_RELEASE_RUN_FAILED\n");exit(1);
}
