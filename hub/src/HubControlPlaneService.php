...
final class HubControlPlaneService
{
    ...
    public function submitCloudTask(string $sessionToken,string $csrfToken,array $payload,?string $now=null):array
    {
        ...
        $projectRow=$this->pdo->prepare('SELECT name,type,provenance FROM projects WHERE project_id=:project');$projectRow->execute(['project'=>$projectId]);$project=$projectRow->fetch();if(!is_array($project)||!str_starts_with((string)($project['provenance']??''),'release-vault:')) throw new HubControlPlaneException('This Cloud review workflow is limited to the release-managed AWH source repository','CLOUD_PROJECT_SOURCE_MISMATCH');
