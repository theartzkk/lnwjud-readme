...
final class HubControlPlaneService
{
    ...
    private function nativeProjectContext(string $userId, string $projectId, string $request = '', ?string $conversationId = null): array
    {
        ...
        $memory = $this->pdo->prepare("SELECT memory_file, status, observed_at FROM project_memory WHERE project_id = :project ORDER BY CASE memory_file WHEN 'CURRENT_STATE.md' THEN 0 ELSE 1 END, memory_file LIMIT 6"); $memory->execute(['project' => $projectId]);
    ...
    private function taskRow(array $row): array
    {
        ...
        if ($this->centralProjectAuthoritySchemaPresent()) {
            $executionQuery = $this->pdo->prepare('SELECT execution_id, executor_kind, required_capability, vault_revision_id, state, lease_owner, cancellation_requested_at, checkpoint_json FROM control_task_executions WHERE task_id = :task');
    ...
    public function publishProjectMemoryFromDevice(string $token, array $payload, ?string $now = null): array
    {
        ...
        $legacy=['ARCHITECTURE.md','DECISIONS.md','HANDOFF.md','PROJECT.md','TASKS.md']; $expected=['ARCHITECTURE.md','CURRENT_STATE.md','DECISIONS.md','HANDOFF.md','PROJECT.md','TASKS.md']; $files=[];
        foreach($payload['files'] as $file){ if(!is_array($file)){ throw new HubControlPlaneException('Project memory metadata is invalid','PAYLOAD_INVALID'); } self::exactKeys($file,['name','sha256','sizeBytes','status']); $name=(string)($file['name']??''); $status=(string)($file['status']??''); $sha=$file['sha256']??null; $size=$file['sizeBytes']??null; if(!in_array($name,$expected,true)||!in_array($status,['present','missing'],true)||!is_int($size)||$size<0||$size>262144||($status==='present'&&(!is_string($sha)||preg_match('/^[0-9a-f]{64}$/i',$sha)!==1))||($status==='missing'&&($sha!==null||$size!==0))) throw new HubControlPlaneException('Project memory metadata is invalid','FIELD_INVALID'); $files[$name]=['status'=>$status,'sha256'=>$sha===null?null:strtolower($sha),'size'=>$size]; }
        $completeCurrent=array_keys(array_intersect_key(array_fill_keys($expected,true),$files))===$expected&&count($files)===count($expected); $completeLegacy=array_keys(array_intersect_key(array_fill_keys($legacy,true),$files))===$legacy&&count($files)===count($legacy); if(!$completeCurrent&&!$completeLegacy) throw new HubControlPlaneException('Project memory metadata is incomplete','PAYLOAD_INVALID');
        $at=self::timestamp($now??gmdate('c')); try{$this->pdo->exec('BEGIN IMMEDIATE'); if($completeCurrent){$this->pdo->prepare('DELETE FROM project_memory WHERE project_id=:project')->execute(['project'=>$projectId]);}else{$this->pdo->prepare("DELETE FROM project_memory WHERE project_id=:project AND memory_file!='CURRENT_STATE.md'")->execute(['project'=>$projectId]);} $q=$this->pdo->prepare('INSERT INTO project_memory(project_id,memory_file,status,sha256,size_bytes,observed_at,provenance) VALUES(:project,:file,:status,:sha,:size,:at,:provenance)'); foreach(($completeCurrent?$expected:$legacy) as $name){$item=$files[$name];$q->execute(['project'=>$projectId,'file'=>$name,'status'=>$item['status'],'sha'=>$item['sha256'],'size'=>$item['status']==='present'?$item['size']:null,'at'=>$at,'provenance'=>'owner-device:canonical-memory-metadata']);} $ready=$this->pdo->prepare("SELECT COUNT(*) FROM project_memory WHERE project_id=:project AND status='present' AND memory_file IN ('CURRENT_STATE.md','PROJECT.md','HANDOFF.md','TASKS.md','ARCHITECTURE.md','DECISIONS.md')"); $ready->execute(['project'=>$projectId]); $memoryReady=(int)$ready->fetchColumn()===6; $this->pdo->exec('COMMIT');}catch(Throwable $error){self::rollbackImmediate($this->pdo); if($error instanceof HubControlPlaneException) throw $error; throw new HubControlPlaneException('Project memory metadata could not be stored','MEMORY_CREATE_FAILED');}
        return ['schemaVersion'=>1,'projectId'=>$projectId,'memoryReady'=>$memoryReady,'observedAt'=>$at];
    }
