...
final class HubStaffOperationsService
{
    ...
    /** @return array<string,int> */
    private function workers(string $at): array { return $this->table('control_workers') ? HubWorkerHealth::counts($this->pdo, $at) : []; }
    ...
    public function persistMorningBrief(array $brief, ?string $now = null): array
    {
        ...
        try {
            ...
            // Morning Brief is a daily durable checkpoint, not a 15-second telemetry ledger.
            // Live health remains available from snapshot()/Infrastructure; a same-day tick
            // must return the existing durable brief even when age/observed byte counters move.
