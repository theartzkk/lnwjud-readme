...
final class HubControlPlaneRouter
{
    ...
    private static function sameOrigin(array $server): void { $origin = $server['HTTP_ORIGIN'] ?? ''; $configured = getenv('AWH_CONTROL_ORIGIN'); if (!is_string($origin) || $origin === '' || !is_string($configured) || $configured === '' || !hash_equals($configured, $origin)) throw new HubControlPlaneException('Browser origin is not authorized', 'ORIGIN_FORBIDDEN'); }
    private static function sameOriginIfBrowser(array $server): void { if (isset($server['HTTP_ORIGIN']) || isset($server['HTTP_SEC_FETCH_SITE'])) self::sameOrigin($server); }
    ...
    private static function exceptionResponse(Throwable $error, string $requestId, array $headers): array { $code = $error instanceof HubControlPlaneException ? $error->codeName : 'CONTROL_PLANE_UNAVAILABLE'; $status = match ($code) { 'ORIGIN_FORBIDDEN', 'CSRF_REJECTED' => 403, 'SESSION_INVALID', 'SESSION_EXPIRED', 'TOKEN_INVALID' => 401, 'PROJECT_FORBIDDEN', 'TASK_FORBIDDEN' => 403, 'TASK_NOT_FOUND' => 404, 'RATE_LIMITED' => 429, 'WORKER_NOT_READY', 'WORKER_BUSY', 'TASK_CLAIM_RACE' => 409, 'SCHEMA_VERSION', 'PAYLOAD_INVALID', 'SCHEMA_FIELDS', 'FIELD_INVALID', 'ID_INVALID', 'GOAL_INVALID', 'IDEMPOTENCY_INVALID', 'PAIRING_CODE_INVALID' => 400, 'PAIRING_REPLAY' => 409, 'CONTROL_SCHEMA_NOT_READY', 'DATABASE_UNAVAILABLE' => 503, default => 400 }; return self::response($status, self::error($code, $requestId, self::safeMessage($code)), $headers); }
