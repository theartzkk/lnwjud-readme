...
final class HubControlPlaneRouter
{
    ...
    private static function exceptionResponse(Throwable $error, string $requestId, array $headers): array { $code = $error instanceof HubControlPlaneException ? $error->codeName : 'CONTROL_PLANE_UNAVAILABLE'; $status = match ($code) { 'ORIGIN_FORBIDDEN', 'CSRF_REJECTED' => 403, 'SESSION_INVALID', 'SESSION_EXPIRED', 'TOKEN_INVALID' => 401, 'PROJECT_FORBIDDEN', 'TASK_FORBIDDEN' => 403, 'TASK_NOT_FOUND', 'APPROVAL_NOT_FOUND' => 404, 'RATE_LIMITED' => 429, 'WORKER_NOT_READY', 'WORKER_BUSY', 'TASK_CLAIM_RACE', 'TASK_UPDATE_RACE', 'TASK_CANCEL_RACE', 'TASK_NOT_CANCELLABLE', 'APPROVAL_ALREADY_DECIDED' => 409, 'SCHEMA_VERSION', 'PAYLOAD_INVALID', 'SCHEMA_FIELDS', 'FIELD_INVALID', 'ID_INVALID', 'GOAL_INVALID', 'IDEMPOTENCY_INVALID', 'PAIRING_CODE_INVALID', 'APPROVAL_DECISION_INVALID' => 400, 'PAIRING_REPLAY', 'APPROVAL_EXPIRED' => 409, 'CONTROL_SCHEMA_NOT_READY', 'ASSISTANT_SCHEMA_NOT_READY', 'DATABASE_UNAVAILABLE' => 503, default => 400 }; return self::response($status, self::error($code, $requestId, self::safeMessage($code)), $headers); }
