...
final class HubControlPlaneRouter
{
    ...
    private static function sameOrigin(array $server): void { $origin = $server['HTTP_ORIGIN'] ?? ''; $configured = self::controlOrigin($server); if (!is_string($origin) || $origin === '' || !is_string($configured) || $configured === '' || !hash_equals($configured, $origin)) throw new HubControlPlaneException('Browser origin is not authorized', 'ORIGIN_FORBIDDEN'); }
    private static function controlOrigin(array $server): string { $requestValue = $server['AWH_CONTROL_ORIGIN'] ?? null; if (is_string($requestValue) && $requestValue !== '') return $requestValue; $environmentValue = getenv('AWH_CONTROL_ORIGIN'); return is_string($environmentValue) ? $environmentValue : ''; }
