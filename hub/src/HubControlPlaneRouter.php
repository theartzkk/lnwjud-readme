    ...
    public static function dispatch(string $method, string $requestUri, array $server, HubControlPlaneService $service, string $body): array
    {
        ...
        try {
            if ($method === 'GET') {
                ...
                if (preg_match('#^/api/v1/control/conversations/(' . self::UUID . ')$#i', $path, $match) === 1) return self::response(200, $service->conversation($sessionToken, $match[1]) + ['requestId' => $requestId], $headers);
