...
final class HubControlPlaneRouter
{
    ...
    public static function dispatch(string $method, string $requestUri, array $server, HubControlPlaneService $service, string $body, array $files = []): array
    {
        ...
        if (!$attachmentUpload && strlen($body) > self::MAX_BODY_BYTES) return self::response(413, self::error('BODY_TOO_LARGE', $requestId, 'Control-plane request is too large'), $headers);
        if ($method === 'POST' && (!$attachmentUpload && (!isset($server['CONTENT_TYPE']) || !is_string($server['CONTENT_TYPE']) || stripos($server['CONTENT_TYPE'], 'application/json') !== 0))) return self::response(415, self::error('CONTENT_TYPE_REQUIRED', $requestId, 'JSON content type is required'), $headers);
    ...
    private static function exceptionResponse(Throwable $error, string $requestId, array $headers): array { $code = $error instanceof HubControlPlaneException ? $error->codeName : 'CONTROL_PLANE_UNAVAILABLE'; $status = match ($code) { 'ORIGIN_FORBIDDEN', 'CSRF_REJECTED', 'WORKSPACE_LEASE_FORBIDDEN', 'ATTACHMENT_FORBIDDEN', 'OWNER_FORBIDDEN', 'STEP_UP_REQUIRED' => 403, 'SESSION_INVALID', 'SESSION_EXPIRED', 'TOKEN_INVALID' => 401, 'PROJECT_FORBIDDEN', 'TASK_FORBIDDEN' => 403, 'TASK_NOT_FOUND', 'CONVERSATION_NOT_FOUND', 'APPROVAL_NOT_FOUND', 'ATTACHMENT_NOT_FOUND' => 404, 'RATE_LIMITED' => 429, 'WORKER_NOT_READY', 'WORKER_BUSY', 'TASK_CLAIM_RACE', 'TASK_UPDATE_RACE', 'TASK_CANCEL_RACE', 'TASK_NOT_CANCELLABLE', 'APPROVAL_ALREADY_DECIDED', 'WORKSPACE_LEASE_HELD', 'WORKSPACE_CHECKPOINT_CONFLICT', 'WORKSPACE_CHECKPOINT_STALE', 'WORKSPACE_CHECKPOINT_REQUIRED', 'PROJECT_ID_CONFLICT' => 409, 'SCHEMA_VERSION', 'PAYLOAD_INVALID', 'SCHEMA_FIELDS', 'FIELD_INVALID', 'ID_INVALID', 'GOAL_INVALID', 'IDEMPOTENCY_INVALID', 'PAIRING_CODE_INVALID', 'APPROVAL_DECISION_INVALID', 'ATTACHMENT_INVALID', 'ATTACHMENT_TYPE_FORBIDDEN', 'PROVIDER_POLICY_INVALID' => 400, 'PAIRING_REPLAY', 'APPROVAL_EXPIRED' => 409, 'CONTROL_SCHEMA_NOT_READY', 'ASSISTANT_SCHEMA_NOT_READY', 'WORKSPACE_SCHEMA_NOT_READY', 'UNIFIED_WORKSPACE_SCHEMA_NOT_READY', 'FINAL_PRODUCT_SCHEMA_NOT_READY', 'ATTACHMENT_STORAGE_UNAVAILABLE', 'ATTACHMENT_STORAGE_FAILED', 'DATABASE_UNAVAILABLE' => 503, default => 400 }; return self::response($status, self::error($code, $requestId, self::safeMessage($code)), $headers); }
    private static function safeMessage(string $code): string { return match ($code) { 'ORIGIN_FORBIDDEN' => 'This AWH surface is not authorized', 'CSRF_REJECTED' => 'The request could not be verified', 'SESSION_INVALID', 'SESSION_EXPIRED' => 'Your AWH session has expired', 'STEP_UP_REQUIRED' => 'Confirm your password again before this change', 'TOKEN_INVALID' => 'Worker authentication failed', 'PROJECT_FORBIDDEN' => 'This project is not available to the current device', 'TASK_NOT_FOUND' => 'Task was not found', 'CONTROL_SCHEMA_NOT_READY' => 'AWH control plane is not activated', 'PAIRING_REPLAY' => 'This connection code was already used', default => 'AWH could not complete the request' }; }
