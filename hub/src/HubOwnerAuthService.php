...
final class HubOwnerAuthService
{
    ...
    public function registerAccessRequest(array $payload, ?string $now = null, ?string $rateKey = null): array
    {
        ...
        $email = self::optionalEmail($payload['email'] ?? null); $phone = self::optionalPhone($payload['phone'] ?? null); $personType = self::personType($payload['personType'] ?? null); self::assertAwhAccountPersonType($personType);
    ...
    public function createUserDirect(string $token, string $csrf, array $payload, ?string $now = null): array
    {
        ...
        $at=self::timestamp($now??gmdate('c'));
        $display=self::displayName($payload['displayName']??null); $username=self::username((string)($payload['username']??'')); self::password((string)($payload['password']??''));
        $email=self::optionalEmail($payload['email']??null); $phone=self::optionalPhone($payload['phone']??null); $person=self::personType($payload['personType']??null); self::assertAwhAccountPersonType($person); $role=self::platformRole($payload['role']??null); $projects=self::projectIds($payload['projectIds']??null,true); self::assertTrustStepUp('account.user.create',$owner,['targetRole'=>$role],$at);
    ...
    public function reviewAccountRequest(string $token,string $csrf,string $requestId,array $payload,?string $now=null): array
    {
        ...
        try{$this->pdo->beginTransaction();$q=$this->pdo->prepare("SELECT * FROM control_account_requests WHERE request_id=:id AND state='PENDING'");$q->execute(['id'=>strtolower($requestId)]);$request=$q->fetch();if(!is_array($request))throw new HubOwnerAuthException('Registration request was not found','REGISTRATION_NOT_FOUND');
            ...
            if(!is_string($request['password_hash'])||$request['password_hash']==='')throw new HubOwnerAuthException('Registration password is unavailable','REGISTRATION_FAILED');self::assertAwhAccountPersonType((string)$request['person_type']);$id=self::uuid();$this->createAccountRecord($id,(string)$request['display_name'],(string)$request['username'],(string)$request['password_hash'],$request['email']===null?null:(string)$request['email'],$request['phone']===null?null:(string)$request['phone'],(string)$request['person_type'],$role,$projects,false,(string)$owner['user_id'],$at);
    ...
    private static function assertAwhAccountPersonType(string $value): void { if(in_array($value,['DIRECTOR','TEACHER','PARENT','STUDENT'],true))throw new HubOwnerAuthException('School identity and role stay in BAY EXCUSE X; create only a KRUART platform account here','IDENTITY_OWNED_BY_BAY'); }
    ...
    private static function platformRole(mixed $value): string { if (!is_string($value) || !in_array(strtoupper($value), ['ADMIN','STAFF','VIEWER'], true)) throw new HubOwnerAuthException('Platform role is invalid', 'PAYLOAD_INVALID'); return strtoupper($value); }
    ...
    private static function roleCapabilities(string $role): array { return match ($role) { 'ADMIN' => ['project.read','conversation.write','attachment.upload','approval.decide','deployment.approve'], 'STAFF' => ['project.read','conversation.write','attachment.upload'], 'VIEWER' => ['project.read'], default => throw new HubOwnerAuthException('Platform role is invalid', 'PAYLOAD_INVALID') }; }
