...
final class HubManagedHostingService
{
    ...
    public function sites(string $token): array
    {
        ...
        $q->execute(['owner'=>$owner['user_id']]); return ['schemaVersion'=>1,'sites'=>array_map([self::class,'siteRow'],$q->fetchAll()),'policy'=>HubTrustPolicy::catalog(['hosting.site.create','hosting.site.deploy','hosting.site.rollback','hosting.site.disable'])];
    }
