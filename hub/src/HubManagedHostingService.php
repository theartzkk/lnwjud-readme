...
final class HubManagedHostingService
{
    ...
    private function ownerMutation(string $token,string $csrf,?string $now): array { $this->ready();try{$row=$this->auth->authorize($token,$csrf,$now);HubOwnerAuthService::assertRecentStepUpSession($row,$now);}catch(HubOwnerAuthException $error){throw new HubManagedHostingException('Hosting authentication needs attention',$error->codeName);}$this->assertOwner((string)$row['user_id']);return $row; }
