...
final class HubManagedHostingService
{
    ...
    private function ecosystemStatus(string $at): array
    {
        ...
        $q=$this->pdo->prepare("SELECT capability,observed_at,expires_at FROM control_executor_capabilities WHERE executor_id='vps-hosting' AND capability LIKE 'hosting.%'");$q->execute();$rows=$q->fetchAll();$fresh=[];$latest=null;foreach($rows as $row){if(is_string($row['observed_at']??null)&&($latest===null||strcmp($row['observed_at'],$latest)>0))$latest=$row['observed_at'];if(is_string($row['expires_at']??null)&&strcmp($row['expires_at'],$at)>0&&is_string($row['capability']??null))$fresh[$row['capability']]=true;}
        $missing=array_values(array_filter($required,static fn($cap)=>!isset($fresh[$cap])));
        ...
        return ['operator'=>['state'=>$missing===[]?'ONLINE':'OFFLINE','freshCapabilities'=>count($fresh),'requiredCapabilities'=>count($required),'missingCapabilities'=>$missing,'lastObservedAt'=>$latest],'tasks'=>$counts,'routing'=>['engine'=>'NGINX','mode'=>'TYPED_CONFIG'],'tls'=>['mode'=>'LETS_ENCRYPT','activation'=>'AFTER_DNS_AND_HEALTH']];
    }
