...
final class HubDatabaseStudioService
{
    ...
    /** Sanitized multi-engine inventory generated by the root-only database inventory timer. */
    public function fleet(string $sessionToken, ?string $now = null): array
    {
        $this->ownerSession($sessionToken, null, null, $now);
        $path = $this->fleetSnapshotPath;
        if (!is_string($path) || $path === '' || str_contains($path, "\0") || !is_file($path) || is_link($path)) {
            return ['schemaVersion' => 1, 'state' => 'NOT_CONFIGURED', 'generatedAt' => null, 'databases' => []];
        }
        try {
            $raw = file_get_contents($path, false, null, 0, 262145);
            if (!is_string($raw) || strlen($raw) > 262144) throw new RuntimeException('snapshot too large');
            $data = json_decode($raw, true, 32, JSON_THROW_ON_ERROR);
            if (!is_array($data) || ($data['schemaVersion'] ?? null) !== 1 || !is_array($data['databases'] ?? null)) throw new RuntimeException('snapshot invalid');
            $rows = [];
            foreach (array_slice($data['databases'], 0, 200) as $row) {
                ...
                $rows[] = [
                    'id' => (string) ($row['id'] ?? strtolower($engine) . ':' . $name),
                    'name' => $name,
                    'label' => self::safeFleetLabel($row['label'] ?? $name),
                    'engine' => $engine,
                    'state' => in_array(($row['state'] ?? ''), ['HEALTHY', 'REVIEW', 'UNAVAILABLE'], true) ? $row['state'] : 'REVIEW',
                    'sizeBytes' => max(0, (int) ($row['sizeBytes'] ?? 0)),
                    'tableCount' => max(0, (int) ($row['tableCount'] ?? 0)),
                    'authority' => in_array(($row['authority'] ?? ''), ['CONTROL', 'STAGING', 'PROOF', 'UNCLASSIFIED'], true) ? $row['authority'] : 'UNCLASSIFIED',
                    'readOnly' => true,
                ];
            }
            return ['schemaVersion' => 1, 'state' => (string) ($data['state'] ?? 'READY'), 'generatedAt' => is_string($data['generatedAt'] ?? null) ? $data['generatedAt'] : null, 'databases' => $rows];
        } catch (Throwable) {
            return ['schemaVersion' => 1, 'state' => 'REVIEW', 'generatedAt' => null, 'databases' => []];
        }
    }
    ...
    private static function safeFleetLabel(mixed $value): string { $value = is_string($value) ? trim($value) : ''; if ($value === '' || strlen($value) > 160 || preg_match('/[\x00-\x1f\x7f]/', $value)) return 'Database'; return $value; }
