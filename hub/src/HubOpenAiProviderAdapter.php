...
final class HubOpenAiProviderAdapter implements HubAiProviderAdapter
{
    ...
    public function call(array $payload, string $credential): array
    {
        ...
        curl_setopt_array($curl,[
            ...
            CURLOPT_HEADERFUNCTION=>static function ($handle, string $line) use (&$retryAfterHeader): int {
                $length = strlen($line);
                $trimmed = trim($line);
                if (preg_match('/^Retry-After\s*:\s*(.+)$/i', $trimmed, $match) === 1) {
        ...
        if ($status < 200 || $status >= 300) {
            $failure=$this->failure($status,is_array($value)?$value:null,$model,$this->retryAfterSeconds($retryAfterHeader));
    ...
    private function failure(int $status, ?array $body, ?string $model, ?int $retryAfterSeconds = null): array
    {
        ...
        return ['code'=>$code,'diagnostic'=>$this->diagnostic($category,$retryable,$status,$type,$providerCode,$model,null,$retryable?$retryAfterSeconds:null)];
    }

    /** @return array<string,mixed> */
    private function diagnostic(string $category,bool $retryable,?int $status,?string $type,?string $code,?string $model,?int $transportCode=null,?int $retryAfterSeconds=null): array
    {
        ...
        if ($retryable && $retryAfterSeconds!==null && $retryAfterSeconds>=1 && $retryAfterSeconds<=3600) $out['retryAfterSeconds']=$retryAfterSeconds;
        return $out;
    }
