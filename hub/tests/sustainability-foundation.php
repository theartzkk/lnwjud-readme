...
$backupRoot = $root . '/backup'; $scratchRoot = $root . '/scratch';
mkdir($backupRoot, 0700, true); mkdir($scratchRoot, 0700, true);
...
try {
    ...
    $sourceHash = hash_file('sha256', $db); expect(is_string($sourceHash), 'source hash missing');
    ...
    expect($latest['status'] === 'VERIFIED', 'latest backup not verified');
    ...
    $manifest = $created['manifest']; $manifest['sha256'] = str_repeat('0', 64);
