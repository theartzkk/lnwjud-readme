...
try {
    ...
    m9_assert($submitted['status'] === 201 && $submittedBody['schemaVersion'] === 3 && count($submittedBody['attachments'] ?? []) === 2 && ($submittedBody['tasks'] ?? []) === [] && count($submittedBody['messages'] ?? []) >= 3, 'a visual question binds image and document inputs then returns an inline conversation response without creating a worker task');
    ...
    $agent = new HubNativeAgentService($pdo, static function (array $payload, string $key) use (&$captured): array { $captured = $payload; m9_assert(str_starts_with($key, 'sk-'), 'provider key stays server side'); return ['output_text' => 'ตรวจภาพแล้ว พร้อมดำเนินการต่อ', 'usage' => ['input_tokens' => 100, 'output_tokens' => 20, 'input_tokens_details' => ['cached_tokens' => 5]]]; }, $fixtureProviderKey);
    ...
    m9_assert($reply['summary'] === 'ตรวจภาพแล้ว พร้อมดำเนินการต่อ' && ($captured['store'] ?? true) === false && str_contains($capturedJson, 'input_image') && str_contains($capturedJson, 'AWH canonical project context') && is_array($fileInput) && ($fileInput['file_data'] ?? null) === base64_encode('AWH document fixture') && !str_contains($capturedJson, $fixtureProviderKey) && !str_contains($capturedJson, $attachmentRoot), 'native provider receives bounded canonical context plus authorized visual and document inputs with no persisted model state, key, or storage path');
