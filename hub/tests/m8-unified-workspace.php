...
try {
    ...
    $memoryFiles = array_map(static fn (string $name): array => ['name' => $name, 'status' => 'present', 'sha256' => str_repeat('a', 64), 'sizeBytes' => 12], ['PROJECT.md', 'HANDOFF.md', 'TASKS.md', 'ARCHITECTURE.md', 'DECISIONS.md']);
    ...
    m8_assert((int) $pdo->query("SELECT count(*) FROM project_memory WHERE project_id = '$project' AND status = 'present'")->fetchColumn() === 5, 'canonical Project Memory metadata has five files without content duplication');
