...
try {
    ...
    $memoryPublish = m8_response($control, 'POST', '/api/v1/control/worker/projects/memory', $deviceAuth, ['schemaVersion' => 1, 'deviceId' => $device, 'projectId' => $project, 'files' => $memoryFiles]);
    m8_assert($memoryPublish['status'] === 200 && str_contains($memoryPublish['body'], '"memoryReady":true'), 'trusted owner device publishes bounded Project Memory metadata');
    m8_assert((int) $pdo->query("SELECT count(*) FROM project_memory WHERE project_id = '$project' AND status = 'present'")->fetchColumn() === 6, 'canonical Project Memory metadata has six files without content duplication');
    $legacyMemoryPublish = m8_response($control, 'POST', '/api/v1/control/worker/projects/memory', $deviceAuth, ['schemaVersion' => 1, 'deviceId' => $device, 'projectId' => $project, 'files' => $legacyMemoryFiles]);
    m8_assert($legacyMemoryPublish['status'] === 200 && str_contains($legacyMemoryPublish['body'], '"memoryReady":true'), 'legacy five-file worker remains compatible after current-state expansion');
    m8_assert((int) $pdo->query("SELECT count(*) FROM project_memory WHERE project_id = '$project' AND memory_file = 'CURRENT_STATE.md' AND status = 'present'")->fetchColumn() === 1, 'legacy worker cannot erase current-state authority');
