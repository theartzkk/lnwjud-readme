...
try {
    ...
    file_put_contents($dir . '/large-tool-read.txt', str_repeat("quoted \\\"text\\\" \\\\\n", 5000) . str_repeat('ก', 20));
    ...
    $internalLarge = $vault->readText($project, $revision, 'src/large-tool-read.txt');
    search_assert(strlen($internalLarge['content']) > HubProjectVault::MAX_TOOL_READ_BYTES && $internalLarge['truncated'] === false, 'internal Vault read keeps its existing larger bound');
    $toolLarge = $vault->toolReadText($project, $revision, 'src/large-tool-read.txt');
    search_assert($toolLarge['truncated'] === true && strlen($toolLarge['content']) <= HubProjectVault::MAX_TOOL_READ_BYTES && preg_match('//u', $toolLarge['content']) === 1, 'provider tool read is bounded and valid UTF-8');
    $toolPayload = json_encode(['data' => $toolLarge], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR);
    search_assert(strlen($toolPayload) < 65536, 'provider tool read stays below the safe JSON result boundary');
