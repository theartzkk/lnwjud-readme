...
$root = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'awh-m4-projects-' . bin2hex(random_bytes(6)); mkdir($root, 0700, true); $db = $root . '/awh.sqlite'; $projectId = '113b45c0-23e1-408d-ae0f-ac5eca7f6900'; $ownerId = '223b45c0-23e1-408d-ae0f-ac5eca7f6900';
try {
    ...
    m4_project_assert(HubControlPlaneProjectRegistration::register($pdo, HubControlPlaneProjectRegistration::PORTABLE_PROJECTS, '2026-08-21T00:00:04.000Z') === 2, 'project registration failed');
    m4_project_assert(HubControlPlaneProjectRegistration::register($pdo, HubControlPlaneProjectRegistration::PORTABLE_PROJECTS, '2026-08-21T00:00:05.000Z') === 2, 'project registration must be idempotent');
    m4_project_assert((int) $pdo->query('SELECT COUNT(*) FROM projects')->fetchColumn() === 3, 'registration must preserve existing AWH project and add two canonical projects');
    m4_project_assert((int) $pdo->query('SELECT COUNT(*) FROM user_project_memberships')->fetchColumn() === 3, 'owner must be a member of all canonical projects');
    fwrite(STDOUT, "AWH M4 project registration tests: PASS\n");
} finally { @unlink($db); @rmdir($root); }
