...
try {
    ...
    $results = m4_response($control, 'GET', '/api/v1/control/results', ['HTTP_ORIGIN' => 'https://awh.test', 'HTTP_COOKIE' => '__Host-awh_control_session=' . $sessionCookie], []);
    $artifacts = m4_response($control, 'GET', '/api/v1/control/artifacts', ['HTTP_ORIGIN' => 'https://awh.test', 'HTTP_COOKIE' => '__Host-awh_control_session=' . $sessionCookie], []);
    m4_assert($results['status'] === 200 && str_contains($results['body'], 'COMPLETED'), 'results route must expose the completed task summary');
    m4_assert($artifacts['status'] === 200 && str_contains($artifacts['body'], 'artifacts'), 'artifacts route must expose bounded metadata');
    $workerResults = m4_response($control, 'GET', '/api/v1/control/worker/results/' . $macId, ['HTTP_AUTHORIZATION' => 'Bearer ' . $worker['accessToken']], []);
    m4_assert($workerResults['status'] === 200 && str_contains($workerResults['body'], 'COMPLETED'), 'worker results route must use device auth without a browser session');
    $approvalTask = m4_response($control, 'POST', '/api/v1/control/tasks', m4_server(['HTTP_ORIGIN' => 'https://awh.test', 'HTTP_COOKIE' => '__Host-awh_control_session=' . $sessionCookie, 'HTTP_X_AWH_CSRF' => $csrf]), ['schemaVersion' => 1, 'projectId' => $projectId, 'goal' => 'แก้ไขไฟล์ตัวอย่างอย่างปลอดภัย', 'idempotencyKey' => 'approval-demo-0001']);
    $approvalTaskPayload = json_decode($approvalTask['body'], true, 32, JSON_THROW_ON_ERROR);
    $approvalClaim = m4_response($control, 'POST', '/api/v1/control/workers/claim', m4_server(['HTTP_AUTHORIZATION' => 'Bearer ' . $worker['accessToken']]), ['schemaVersion' => 1, 'deviceId' => $macId]);
    $approvalClaimPayload = json_decode($approvalClaim['body'], true, 32, JSON_THROW_ON_ERROR);
    m4_assert(($approvalClaimPayload['task']['taskId'] ?? null) === ($approvalTaskPayload['taskId'] ?? null), 'worker must claim the approval-scoped task');
    $needsApproval = m4_response($control, 'POST', '/api/v1/control/tasks/' . $approvalTaskPayload['taskId'] . '/update', m4_server(['HTTP_AUTHORIZATION' => 'Bearer ' . $worker['accessToken']]), ['schemaVersion' => 1, 'deviceId' => $macId, 'state' => 'WAITING_FOR_APPROVAL', 'progress' => 20, 'message' => 'bounded approval requested', 'resultSummary' => null]);
    m4_assert($needsApproval['status'] === 200 && str_contains($needsApproval['body'], 'PENDING'), 'worker approval transition must create a scoped approval');
    $approvals = m4_response($control, 'GET', '/api/v1/control/approvals', ['HTTP_ORIGIN' => 'https://awh.test', 'HTTP_COOKIE' => '__Host-awh_control_session=' . $sessionCookie], []);
    $approvalPayload = json_decode($approvals['body'], true, 32, JSON_THROW_ON_ERROR);
    $approvalId = $approvalPayload['approvals'][0]['approvalId'] ?? null;
    m4_assert($approvals['status'] === 200 && is_string($approvalId) && preg_match('/^[0-9a-f]{64}$/', (string) ($approvalPayload['approvals'][0]['scope']['goalDigest'] ?? '')) === 1, 'approval list must expose only the bounded approval metadata');
    $decision = m4_response($control, 'POST', '/api/v1/control/approvals/' . $approvalId . '/approve', m4_server(['HTTP_ORIGIN' => 'https://awh.test', 'HTTP_COOKIE' => '__Host-awh_control_session=' . $sessionCookie, 'HTTP_X_AWH_CSRF' => $csrf]), ['schemaVersion' => 1]);
    m4_assert($decision['status'] === 200 && str_contains($decision['body'], 'APPROVED'), 'approval decision must be idempotent and requeue the task');
