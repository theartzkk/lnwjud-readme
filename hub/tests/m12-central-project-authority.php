...
try {
    ...
    $historyAgent->respond($owner, $project, null, null, 'คำถามล่าสุด', [['role' => 'user', 'body' => 'คำถามก่อนหน้า'], ['role' => 'assistant', 'body' => 'คำตอบก่อนหน้า']], [], $now, []);
    ...
    $agentExecutor = new HubDurableExecutionService($pdo, $vaults, $agent, HubArtifactStore::fromEnvironment()); $agentExecutor->enqueue($successTask, $project, (string) $promoted['activeRevisionId'], 'VPS', 'agent.conversation', ['mode' => 'NATIVE_CONVERSATION', 'messageId' => $successMessage], $now);
