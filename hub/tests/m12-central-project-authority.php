...
try {
    ...
    // A canonical deployed-source promotion supersedes any still-pending
    // candidate approval that was captured against the previous active revision.
    $staleBase = (string) $vaults->activeRevision($project); $staleTask = m12_uuid();
    $pdo->prepare("INSERT INTO control_tasks(task_id,user_id,project_id,goal,state,assigned_device_id,lease_expires_at,progress,result_summary,failure_code,idempotency_key,conversation_id,created_at,updated_at,cancelled_at) VALUES(:task,:user,:project,'stale approval fixture','WAITING_FOR_APPROVAL',NULL,NULL,90,'waiting for candidate approval',NULL,:key,NULL,:at,:at,NULL)")->execute(['task'=>$staleTask,'user'=>$owner,'project'=>$project,'key'=>'m12-stale-approval-0001','at'=>$now]);
    $staleArchive = $root . '/stale-candidate.zip'; $zip = new ZipArchive(); m12_assert($zip->open($staleArchive, ZipArchive::CREATE) === true, 'stale candidate zip create'); $zip->addFromString('README.md', "# stale candidate\n"); $zip->addFromString('src/main.php', "<?php echo 'stale-candidate';\n"); $zip->close();
    $staleCandidate = $vaults->captureTaskArchive($project, $staleArchive, $owner, $staleTask, $staleBase, $now);
    $orphanArchive = $root . '/orphan-archive.zip'; $zip = new ZipArchive(); m12_assert($zip->open($orphanArchive, ZipArchive::CREATE) === true, 'orphan archive zip create'); $zip->addFromString('README.md', "# orphan archive candidate\n"); $zip->addFromString('src/main.php', "<?php echo 'orphan-archive';\n"); $zip->close();
    $orphanCandidate = $vaults->ingestArchive($project, $orphanArchive, $owner, null, $staleBase, $now); m12_assert(($orphanCandidate['promotionRequired'] ?? false) === true, 'orphan archive remains an explicit candidate before canonical refresh');
    $staleApproval = m12_uuid(); $staleScope = json_encode(['taskId'=>$staleTask,'projectId'=>$project,'expectedActiveRevisionId'=>$staleBase,'candidateRevisionId'=>$staleCandidate['revisionId'],'artifactId'=>m12_uuid()], JSON_THROW_ON_ERROR);
    $pdo->prepare("INSERT INTO control_approvals(approval_id,task_id,action,scope_json,status,expires_at,decided_at) VALUES(:approval,:task,'project.revision.promote',:scope,'PENDING',:expires,NULL)")->execute(['approval'=>$staleApproval,'task'=>$staleTask,'scope'=>$staleScope,'expires'=>gmdate('c', strtotime($now)+3600)]);
    $deployArchive = $root . '/canonical-refresh.zip'; $zip = new ZipArchive(); m12_assert($zip->open($deployArchive, ZipArchive::CREATE) === true, 'canonical refresh zip create'); $zip->addFromString('README.md', "# canonical refresh\n"); $zip->addFromString('src/main.php', "<?php echo 'canonical-refresh';\n"); $zip->close();
    $refresh = $vaults->ingestArchive($project, $deployArchive, $owner, null, $staleBase, $now); m12_assert(($refresh['promotionRequired'] ?? false) === true, 'canonical refresh is captured as a revision-checked candidate');
    $refreshed = $vaults->promote($project, (string) $refresh['createdRevisionId'], $staleBase, $now);
    $reconciled = $vaults->expireStalePromotionApprovals($project, (string) $refreshed['activeRevisionId'], $now); m12_assert(($reconciled['expiredApprovals'] ?? 0) >= 1 && ($reconciled['rejectedCandidates'] ?? 0) >= 2, 'canonical promotion expires stale approvals and rejects every candidate bound to an older active revision');
    $staleApprovalRow = $pdo->prepare('SELECT status,decided_at FROM control_approvals WHERE approval_id=:approval'); $staleApprovalRow->execute(['approval'=>$staleApproval]); $staleApprovalState=$staleApprovalRow->fetch();
    $staleTaskRow = $pdo->prepare('SELECT state,progress,failure_code,result_summary,cancelled_at FROM control_tasks WHERE task_id=:task'); $staleTaskRow->execute(['task'=>$staleTask]); $staleTaskState=$staleTaskRow->fetch();
    $staleCandidateRow = $pdo->prepare('SELECT state FROM control_project_vault_revisions WHERE revision_id=:revision'); $staleCandidateRow->execute(['revision'=>$staleCandidate['revisionId']]);
    $staleEvent = $pdo->prepare("SELECT state,progress,message FROM control_task_events WHERE task_id=:task ORDER BY occurred_at DESC,event_id DESC LIMIT 1"); $staleEvent->execute(['task'=>$staleTask]); $staleEventState=$staleEvent->fetch();
    m12_assert(($staleApprovalState['status'] ?? null) === 'EXPIRED' && is_string($staleApprovalState['decided_at'] ?? null), 'stale promotion approval becomes non-actionable');
    m12_assert(($staleTaskState['state'] ?? null) === 'CANCELLED' && (int)($staleTaskState['progress'] ?? 0) === 100 && ($staleTaskState['failure_code'] ?? null) === null && str_contains((string)($staleTaskState['result_summary'] ?? ''), 'canonical revision ใหม่กว่า') && is_string($staleTaskState['cancelled_at'] ?? null), 'superseded candidate task closes without a false failure');
    m12_assert($staleCandidateRow->fetchColumn() === 'REJECTED', 'superseded candidate revision is no longer promotable');
    $orphanState = $pdo->prepare('SELECT state FROM control_project_vault_revisions WHERE revision_id=:revision'); $orphanState->execute(['revision'=>$orphanCandidate['createdRevisionId']]);
    m12_assert($orphanState->fetchColumn() === 'REJECTED', 'orphan archive candidate from an older canonical base is reconciled automatically');
    $vaultAfterReconcile = $vaults->state($project); m12_assert(($vaultAfterReconcile['syncState'] ?? null) === 'SYNCED', 'canonical Vault returns to SYNCED when no current-base candidates remain');
    m12_assert(($staleEventState['state'] ?? null) === 'CANCELLED' && (int)($staleEventState['progress'] ?? 0) === 100, 'stale approval reconciliation leaves a truthful terminal event');
