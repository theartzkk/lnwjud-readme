...
try {
    ...
    $brandLogo = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=';
    $brandSaved = m11_body(m11_control($control, 'POST', '/api/v1/control/settings', $browser, ['schemaVersion' => 2, 'settingKey' => 'brandLogoDataUrl', 'value' => $brandLogo]));
    m11_assert(($brandSaved['settings']['brandLogoDataUrl']['value'] ?? null) === $brandLogo && (int)($brandSaved['settings']['brandLogoDataUrl']['revision'] ?? 0) === 1, 'brand logo is stored in the existing revisioned product settings authority');
    $brandInvalid = m11_control($control, 'POST', '/api/v1/control/settings', $browser, ['schemaVersion' => 2, 'settingKey' => 'brandIconDataUrl', 'value' => 'data:image/png;base64,' . base64_encode('not-a-real-png-payload')]);
    m11_assert($brandInvalid['status'] === 400 && str_contains($brandInvalid['body'], 'FIELD_INVALID'), 'brand image rejects payloads whose bytes do not match the declared image type');
    $brandOversize = m11_control($control, 'POST', '/api/v1/control/settings', $browser, ['schemaVersion' => 2, 'settingKey' => 'brandIconDataUrl', 'value' => 'data:image/png;base64,' . base64_encode("\x89PNG\r\n\x1a\n" . str_repeat('x', 9100))]);
    m11_assert($brandOversize['status'] === 400 && str_contains($brandOversize['body'], 'FIELD_INVALID'), 'brand image remains bounded below the control JSON body ceiling');
    $brandReset = m11_body(m11_control($control, 'POST', '/api/v1/control/settings/reset', $browser, ['schemaVersion' => 2, 'settingKey' => 'brandLogoDataUrl']));
    m11_assert(array_key_exists('value', $brandReset['settings']['brandLogoDataUrl'] ?? []) && $brandReset['settings']['brandLogoDataUrl']['value'] === null && (int)$brandReset['settings']['brandLogoDataUrl']['revision'] === 2, 'brand logo reset is reversible and revisioned');
