...
$insertTask->execute(['task'=>'t-source','goal'=>'managed hosting source binding']);
$insertExecution->execute(['id'=>'e-source','task'=>'t-source','state'=>'WAITING_FOR_CAPABILITY','capability'=>'hosting.site.provision','attempts'=>0,'code'=>'PROJECT_SOURCE_NOT_READY','checkpoint'=>'{}','updated'=>'2026-09-02T09:20:00+00:00']);
...
failure_policy_assert(($snapshot['current']['total'] ?? null) === 2, 'current blocker projection excludes superseded and non-alerting policy-paused work');
failure_policy_assert(($snapshot['summary']['obsoleteStale'] ?? null) === 1, 'superseded failure remains audit-only stale evidence');
failure_policy_assert(($snapshot['summary']['policyPaused'] ?? null) === 1 && ($snapshot['nonAlerting']['policyPausedCount'] ?? null) === 1, 'quota wait remains visible but non-alerting');
failure_policy_assert(($snapshot['current']['summary']['setupRequired'] ?? null) === 1, 'missing project source is one actionable setup requirement');
failure_policy_assert(($snapshot['current']['summary']['blockedCapability'] ?? null) === 0, 'policy pause is never inflated into a capability blocker');
