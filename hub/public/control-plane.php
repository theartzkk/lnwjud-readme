...
try {
    ...
    $candidateFile = [];
    if ($workerCandidate) {
        $length = isset($_SERVER['CONTENT_LENGTH']) ? filter_var($_SERVER['CONTENT_LENGTH'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 1024 * 1024 * 1024]]) : false;
        ...
        if (!is_int($copied) || $copied !== $length || !is_string($tmpPath) || !is_file($tmpPath)) { fclose($temporary); throw new RuntimeException('Candidate upload is incomplete'); }
        fflush($temporary); $candidateFile = ['candidate' => ['tmp_name' => $tmpPath, 'size' => $length]]; $body = '';
    ...
    if (str_starts_with($path, '/api/v1/auth/')) {
        ...
        $response = HubControlPlaneRouter::dispatch((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET'), (string) ($_SERVER['REQUEST_URI'] ?? '/'), $_SERVER, $control, is_string($body) ? $body : '', $workerCandidate ? $candidateFile : $_FILES);
    }
