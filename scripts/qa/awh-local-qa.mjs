...
async function gitCheck() {
  ...
  const valid = branch.code === 0 && head.code === 0 && status.code === 0 && validHead && validBranch;
  check('git-state', valid ? 'PASS' : 'FAIL', valid ? `branch=${branchName}; HEAD=${head.stdout}; dirty=${Boolean(status.stdout)}` : 'local Git branch/HEAD/status could not be verified', started);
...
async function phpHubCheck() {
  ...
  const m12Syntax = ['hub/src/HubCentralProjectAuthorityMigration.php', 'hub/src/HubProjectVault.php', 'hub/src/HubProjectVaultService.php', 'hub/src/HubDurableExecutionService.php', 'hub/bin/migrate-central-project-authority.php', 'hub/bin/awh-native-executor.php', 'hub/tests/m12-central-project-authority.php', 'hub/tests/m12-production-parity.php'];
