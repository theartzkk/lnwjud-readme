...
async function requiredFilesCheck() {
  ...
  const m12Required = [
    ...
    'deploy/systemd/awh-native-executor.service', 'deploy/systemd/awh-native-executor.timer', 'deploy/awh-control-plane/verify-web-release.php', 'deploy/awh-control-plane/validate-remote-output.sh', 'test/central-project-authority-deployment.test.ts', 'test/final-uat-shell.test.ts',
  ];
  const missing = [];
...
async function finalUatShellCheck() {
  const started = Date.now();
  const result = await runNodeTest(['test/final-uat-shell.test.ts'], 90_000);
  check('final-uat-shell', result.code === 0 ? 'PASS' : 'FAIL', result.code === 0 ? 'mobile scroll, shared navigation, manifest and typed deploy output regressions passed' : `final UAT shell contracts failed with exit code ${result.code}`, started);
}
