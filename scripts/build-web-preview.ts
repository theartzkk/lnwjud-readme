...
function withDashboard(index: string): string {
  ...
  return index
    ...
    .replace('</body>', '  <script type="module" src="./dashboard.js?release=__AWH_WEB_RELEASE_ID__"></script>\n</body>');
}
...
async function main(): Promise<void> {
  ...
  const [index, styles, app, dashboardCss, dashboardJs, hubAdapter, controlAdapter, manifest, serviceWorker, databaseHtml, databaseCss, databaseJs] = await Promise.all([
    asset('index.html'), asset('styles.css'), asset('app.js'), asset('dashboard.css'), asset('dashboard.js'), asset('hub-read-adapter.js'), asset('control-plane-adapter.js'), asset('manifest.webmanifest'), asset('sw.js'), asset('database.html'), asset('database.css'), asset('database.js'),
  ]);
