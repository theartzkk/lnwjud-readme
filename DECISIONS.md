...
- M6 Native Assistant Workstream is a prepared additive capability, not a live-production claim. It raises the shared Hub database from v5 to v6 only after a new exact release approval. Its durable conversation is an ordered projection over M4 tasks/events/artifacts/approvals, not another assistant memory, project registry, task engine or result store.
