...
# Authoritative decision — 2026-08-30 provider-failure continuity and live reconciliation

- Live Production is determined from fresh read-only evidence, not historical checkpoint text: ReadyIDC currently points Control/Web at `m16-12fb67a09d4b` / source marker `12fb67a09d4b87e942c8aa120e40e9c8d33d1f9f`, with active Nginx/PHP-FPM/executor/backup timers and healthy schema-16 integrity/FK/backup evidence.
- `AUTOCHAIN_FIELD_TIMEOUT` after correct canonical `project.read` routing is a shared continuity problem when the provider returns `PROVIDER_FAILED`; it is not permission to declare success, inject a token, or create a second queue. The permanent source fix is the existing executor's bounded deterministic Vault fallback for safe read-only inspection, while the failed provider outcome remains recorded.
- Canonical source `bd6acaa664043d8fc26e26975f526482e9fc3159` is included in isolated candidate `66c86584f299433f791c0fceb8e24b4348b94aef` before this documentation checkpoint. Morning Brief persistence uses the existing settings-revision ledger and now fingerprints material state so it does not remain stale for an entire day.
- Any final candidate hash after this checkpoint requires fresh exact-head CI and, if Production activation is needed, one exact-SHA Owner approval through typed M16. Historical approvals must not be reused across source or documentation changes.
