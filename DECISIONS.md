...
- `KRUART_ECOSYSTEM_MASTER_CONTROL.md` is the durable cross-project lane registry. Its purpose is to prevent conversational memory, stale handoffs, or parallel chats from becoming accidental orchestration authority.
- One Main Orchestration Chat coordinates the ecosystem, while each active lane may have only one mutation owner at a time. A different chat may inspect or prepare evidence but must not mutate that lane until ownership is handed off.
- A lane is not considered closed without a safe checkpoint and FINAL HANDOFF or an equivalent durable Master Control update containing exact source/runtime/QA/backup/blocker/next-action evidence.
- AWH `CURRENT_STATE.md` remains the AWH operational-state authority and fresh live evidence still overrides dated records; the Master Control coordinates ownership/status rather than replacing per-project Source of Truth.
...
- While the BAY EXCUSE X → VPS migration lane is ACTIVE, that lane alone owns its migration mutations. Other chats must consume its handoff rather than repeat capture, restore, deploy, DNS cutover, database mutation, or LINE/LIFF migration work.
