...
if test "$ACTION" = rollback; then
  ...
  O=$(sudo -n stat -c '%u' "$B") || exit 1
  G=$(sudo -n stat -c '%g' "$B") || exit 1
  M=$(sudo -n stat -c '%a' "$B") || exit 1
...
rollback(){
  ...
  if test "$code" -eq 0 || test "$MUTATED" -ne 1; then
    sudo -n rm -f "$T" "$B" >/dev/null 2>&1 || true
    exit "$code"
  fi
  if sudo -n test -f "$B" && sudo -n cp -p "$B" "$T" && sudo -n mv -f "$T" "$F" && sudo -n nginx -t >/dev/null 2>&1 && sudo -n systemctl reload nginx >/dev/null 2>&1; then
    printf '%s\n' 'ROLLBACK=PASS'
  else
    printf '%s\n' 'ROLLBACK=FAIL'
  fi
...
for P in / /api/v1/health /api/v1/status /api/v1/projects; do
  ...
  test "$C" = 401 || fail AUTH_VERIFY UNAUTHENTICATED_PERIMETER
done
stage COMPLETE
printf '%s\n' 'ROTATE_RESULT=PASS'
