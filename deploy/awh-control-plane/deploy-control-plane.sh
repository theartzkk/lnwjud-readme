...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "Usage: $0 [--dry-run] | --deploy --approve --owner-auth [--compat-refresh|--assistant-workstream|--workspace-continuity|--unified-workspace|--final-product|--founding-memory|--self-service|--central-project-authority|--anywhere-execution|--cost-aware-ai|--automations|--self-sufficient-ai] [--cleanup-topology]" >&2; exit 2 ;;
  esac
done
...
if test "$MODE" = dry-run; then
  ...
  if test "$SELF_SUFFICIENT_AI" -eq 1; then
...
if test $((COMPAT_REFRESH + ASSISTANT_WORKSTREAM + WORKSPACE_CONTINUITY + UNIFIED_WORKSPACE + FINAL_PRODUCT + FOUNDING_MEMORY + SELF_SERVICE + CENTRAL_PROJECT_AUTHORITY + ANYWHERE_EXECUTION + COST_AWARE_AI + AUTOMATIONS + SELF_SUFFICIENT_AI)) -gt 1; then echo "Choose one bounded activation mode" >&2; exit 2; fi
...
OWNER_PASSWORD=
if test "$ASSISTANT_WORKSTREAM" -eq 0 && test "$WORKSPACE_CONTINUITY" -eq 0 && test "$UNIFIED_WORKSPACE" -eq 0 && test "$FINAL_PRODUCT" -eq 0 && test "$FOUNDING_MEMORY" -eq 0 && test "$SELF_SERVICE" -eq 0 && test "$CENTRAL_PROJECT_AUTHORITY" -eq 0 && test "$ANYWHERE_EXECUTION" -eq 0 && test "$COST_AWARE_AI" -eq 0 && test "$AUTOMATIONS" -eq 0 && test "$SELF_SUFFICIENT_AI" -eq 0; then
...
EXTRA_FILES=
if test "$CENTRAL_PROJECT_AUTHORITY" -eq 1 || test "$ANYWHERE_EXECUTION" -eq 1 || test "$COST_AWARE_AI" -eq 1 || test "$AUTOMATIONS" -eq 1 || test "$SELF_SUFFICIENT_AI" -eq 1; then
  command -v git >/dev/null 2>&1 || { echo "git is required to build the M12/M13/M14/M15/M16 source snapshot" >&2; exit 1; }
  ...
  test -s "$ROOT/.awh-build/awh-source.zip" || { echo "M12/M13/M14/M15/M16 exact source snapshot is unavailable" >&2; exit 1; }
...
set +e
if test "$ASSISTANT_WORKSTREAM" -eq 1 || test "$WORKSPACE_CONTINUITY" -eq 1 || test "$UNIFIED_WORKSPACE" -eq 1 || test "$FINAL_PRODUCT" -eq 1 || test "$FOUNDING_MEMORY" -eq 1 || test "$SELF_SERVICE" -eq 1 || test "$CENTRAL_PROJECT_AUTHORITY" -eq 1 || test "$ANYWHERE_EXECUTION" -eq 1 || test "$COST_AWARE_AI" -eq 1 || test "$AUTOMATIONS" -eq 1 || test "$SELF_SUFFICIENT_AI" -eq 1; then
  REMOTE_OUTPUT=$(ssh -o BatchMode=yes -o StrictHostKeyChecking=yes "$TARGET" sh "$REMOTE_SCRIPT" "$DB_PATH" "$REMOTE_ROOT" "$REMOTE_STAGE" "/opt/awh-hub/control-releases/$RELEASE_ID" "$RELEASE_ID" "$NGINX_CONFIG" "$HOSTNAME" "$AWH_FPM_SOCKET" "$AWH_FPM_SERVICE" "$CLEANUP_TOPOLOGY" "$OWNER_USERNAME" "$OWNER_AUTH" "$REMOTE_SCRIPT" "$COMPAT_REFRESH" "$ASSISTANT_WORKSTREAM" "$WORKSPACE_CONTINUITY" "$UNIFIED_WORKSPACE" "$FINAL_PRODUCT" "$FOUNDING_MEMORY" "$SELF_SERVICE" "$CENTRAL_PROJECT_AUTHORITY" "$RELEASE" "$ANYWHERE_EXECUTION" "$COST_AWARE_AI" "$AUTOMATIONS" "$SELF_SUFFICIENT_AI")
else
