...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "Usage: $0 [--dry-run] | --deploy --approve --owner-auth [--compat-refresh|--assistant-workstream|--workspace-continuity|--unified-workspace|--final-product|--founding-memory|--self-service|--central-project-authority|--anywhere-execution|--cost-aware-ai|--automations|--self-sufficient-ai|--account-hosting] [--cleanup-topology]" >&2; exit 2 ;;
  esac
done
...
if test "$ACCOUNT_HOSTING" -eq 1; then RELEASE_ID=m17-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$SELF_SUFFICIENT_AI" -eq 1; then RELEASE_ID=m16-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$AUTOMATIONS" -eq 1; then RELEASE_ID=m15-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$COST_AWARE_AI" -eq 1; then RELEASE_ID=m14-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$ANYWHERE_EXECUTION" -eq 1; then RELEASE_ID=m13-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$CENTRAL_PROJECT_AUTHORITY" -eq 1; then RELEASE_ID=m12-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$SELF_SERVICE" -eq 1; then RELEASE_ID=m11-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$FOUNDING_MEMORY" -eq 1; then RELEASE_ID=m10-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$FINAL_PRODUCT" -eq 1; then RELEASE_ID=m9-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$UNIFIED_WORKSPACE" -eq 1; then RELEASE_ID=m8-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$WORKSPACE_CONTINUITY" -eq 1; then RELEASE_ID=m7-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$ASSISTANT_WORKSTREAM" -eq 1; then RELEASE_ID=m6-$(printf '%s' "$RELEASE" | cut -c1-12); else RELEASE_ID=m4-$(printf '%s' "$RELEASE" | cut -c1-12); fi
...
FILES="$FILES hub/src/HubStorageGovernanceService.php hub/src/HubExecutionTriageService.php hub/src/HubStaffGovernorService.php hub/src/HubStaffOperationsService.php deploy/awh-control-plane/verify-web-release.php dist-web/navigation.js"
FILES="$FILES dist-web/responsive-layout.css"
DESKTOP_ARTIFACTS=
...
if test "$MODE" = dry-run; then
  ...
  if test "$ACCOUNT_HOSTING" -eq 1; then
...
if test $((COMPAT_REFRESH + ASSISTANT_WORKSTREAM + WORKSPACE_CONTINUITY + UNIFIED_WORKSPACE + FINAL_PRODUCT + FOUNDING_MEMORY + SELF_SERVICE + CENTRAL_PROJECT_AUTHORITY + ANYWHERE_EXECUTION + COST_AWARE_AI + AUTOMATIONS + SELF_SUFFICIENT_AI + ACCOUNT_HOSTING)) -gt 1; then echo "Choose one bounded activation mode" >&2; exit 2; fi
...
OWNER_PASSWORD=
if test "$ASSISTANT_WORKSTREAM" -eq 0 && test "$WORKSPACE_CONTINUITY" -eq 0 && test "$UNIFIED_WORKSPACE" -eq 0 && test "$FINAL_PRODUCT" -eq 0 && test "$FOUNDING_MEMORY" -eq 0 && test "$SELF_SERVICE" -eq 0 && test "$CENTRAL_PROJECT_AUTHORITY" -eq 0 && test "$ANYWHERE_EXECUTION" -eq 0 && test "$COST_AWARE_AI" -eq 0 && test "$AUTOMATIONS" -eq 0 && test "$SELF_SUFFICIENT_AI" -eq 0 && test "$ACCOUNT_HOSTING" -eq 0; then
...
EXTRA_FILES=
if test "$CENTRAL_PROJECT_AUTHORITY" -eq 1 || test "$ANYWHERE_EXECUTION" -eq 1 || test "$COST_AWARE_AI" -eq 1 || test "$AUTOMATIONS" -eq 1 || test "$SELF_SUFFICIENT_AI" -eq 1 || test "$ACCOUNT_HOSTING" -eq 1; then
  command -v git >/dev/null 2>&1 || { echo "git is required to build the M12/M13/M14/M15/M16/M17 source snapshot" >&2; exit 1; }
...
set +e
if test "$ASSISTANT_WORKSTREAM" -eq 1 || test "$WORKSPACE_CONTINUITY" -eq 1 || test "$UNIFIED_WORKSPACE" -eq 1 || test "$FINAL_PRODUCT" -eq 1 || test "$FOUNDING_MEMORY" -eq 1 || test "$SELF_SERVICE" -eq 1 || test "$CENTRAL_PROJECT_AUTHORITY" -eq 1 || test "$ANYWHERE_EXECUTION" -eq 1 || test "$COST_AWARE_AI" -eq 1 || test "$AUTOMATIONS" -eq 1 || test "$SELF_SUFFICIENT_AI" -eq 1 || test "$ACCOUNT_HOSTING" -eq 1; then
