...
FILES="$FILES deploy/awh-control-plane/install-database-studio-mariadb.sh"
FILES="$FILES deploy/nginx/awh-edge-hardening.conf deploy/nginx/awh-legacy-control-compat.conf deploy/fail2ban/awh-nginx-botsearch.local deploy/fail2ban/awh-nginx-scanner.conf deploy/awh-control-plane/install-edge-hardening.sh"
...
FILES="$FILES hub/src/HubVaultSourceAuthorityMigration.php hub/migrations/020_vault_source_authority.sql hub/bin/migrate-vault-source-authority.php hub/bin/bind-vault-source-authority.php hub/bin/reconcile-vault-source-authority.php"
FILES="$FILES hub/src/HubIdentityConvergenceMigration.php hub/src/HubBaySchoolAuthorityConnector.php hub/src/HubSchoolIdentityService.php hub/migrations/021_identity_convergence.sql hub/bin/migrate-identity-convergence.php"
...
FILES="$FILES hub/src/HubUpdateTargetRegistry.php hub/src/HubOperatorBridgeService.php hub/bin/awh-operator-bridge.php deploy/operator-bridge/awh-operator deploy/operator-bridge/install.sh deploy/systemd/awh-operator-bridge.socket deploy/systemd/awh-operator-bridge@.service"
FILES="$FILES hub/src/HubAssessmentReleaseService.php hub/src/HubAssessmentReleaseOperator.php hub/bin/awh-assessment-release-run.php hub/bin/register-managed-products.php deploy/assessment/awh-assessment-release-engine.py"
FILES="$FILES deploy/remote-worker/linux/bootstrap-vps-direct-connector.sh deploy/remote-worker/linux/install-node-runtime.sh deploy/remote-worker/linux/install-vps-direct-connector.sh deploy/remote-worker/linux/verify-vps-direct-connector.sh deploy/remote-worker/linux/desktop-commander-vps.service.template deploy/qa/install-browser-qa-runtime.sh deploy/qa/verify-browser-qa-runtime.sh scripts/qa/run-vps-chat-continuity.sh"
DESKTOP_ARTIFACTS=
if test "$REUSE_REMOTE_DESKTOP_ARTIFACTS" -eq 1; then
...
test -f "$DURABLE_RUNNER" || { echo "Missing durable remote deployment runner" >&2; exit 1; }
...
if test "$MODE" = dry-run; then
  ...
  if test "$IDENTITY_CONVERGENCE" -eq 1; then
    echo "M22_ROLLBACK=restore-exact-db-baseline,pointer,web-pointer,nginx,service-health,m3d-m3e-control-regression"
    echo "M22_PRODUCTION_ACTIVATION_REQUIRES_APPROVAL"
  elif test "$PROJECT_SOURCE_AUTHORITY" -eq 1; then
    echo "M21_ROLLBACK=restore-exact-db-baseline,pointer,web-pointer,native-executor,hosting-operator,nginx,service-health,m3d-m3e-control-regression"
    echo "M21_PRODUCTION_ACTIVATION_REQUIRES_APPROVAL"
  elif test "$CONVERSATION_LIFECYCLE" -eq 1; then
    echo "M19_ROLLBACK=restore-exact-db-baseline,pointer,web-pointer,native-executor,hosting-operator,nginx,service-health,m3d-m3e-control-regression"
    echo "M19_PRODUCTION_ACTIVATION_REQUIRES_APPROVAL"
  elif test "$CLOUD_FIRST" -eq 1; then
...
if test "$OWNER_LOGIN_PROOF_REQUIRED" -eq 0; then
  ...
  if test "$REMOTE_STATUS" -eq 124; then REMOTE_OUTPUT='DEPLOY_FAILED_AT=REMOTE_TRANSPORT'; fi
else
