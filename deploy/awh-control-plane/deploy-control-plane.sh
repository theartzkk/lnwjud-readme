...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "Usage: $0 [--dry-run] | --deploy --approve --owner-auth [--compat-refresh|--assistant-workstream|--workspace-continuity] [--cleanup-topology]" >&2; exit 2 ;;
  esac
done
...
if test "$WORKSPACE_CONTINUITY" -eq 1; then RELEASE_ID=m7-$(printf '%s' "$RELEASE" | cut -c1-12); elif test "$ASSISTANT_WORKSTREAM" -eq 1; then RELEASE_ID=m6-$(printf '%s' "$RELEASE" | cut -c1-12); else RELEASE_ID=m4-$(printf '%s' "$RELEASE" | cut -c1-12); fi
...
if test "$MODE" = dry-run; then
  ...
  if test "$WORKSPACE_CONTINUITY" -eq 1; then
...
if test $((COMPAT_REFRESH + ASSISTANT_WORKSTREAM + WORKSPACE_CONTINUITY)) -gt 1; then echo "Choose one bounded activation mode" >&2; exit 2; fi
...
OWNER_PASSWORD=
if test "$ASSISTANT_WORKSTREAM" -eq 0 && test "$WORKSPACE_CONTINUITY" -eq 0; then
