...
for arg in "$@"; do
  case "$arg" in
    ...
    --deploy) MODE=deploy ;;
    --approve) APPROVED=1 ;;
    *) echo "Usage: $0 [--dry-run] | --deploy --approve" >&2; exit 2 ;;
  esac
done
...
if test "$MODE" = dry-run; then
  ...
  echo "M4_PLAN=preflight,backup,stage,migrate-003,idempotence,register-projects,control-pointer,nginx-test,reload,read-regression"
...
PREFLIGHT_OUTPUT=$(AWH_DEPLOY_TARGET="$TARGET" sh "$PREFLIGHT") || { echo "M4 preflight failed" >&2; exit 1; }
...
set +e
REMOTE_OUTPUT=$(ssh -o BatchMode=yes -o StrictHostKeyChecking=yes "$TARGET" sh -s -- "$DB_PATH" "$REMOTE_ROOT" "$REMOTE_STAGE" "/opt/awh-hub/control-releases/$RELEASE_ID" "$RELEASE_ID" "$NGINX_CONFIG" "$PHP_VERSION" "$HOSTNAME" < "$REMOTE_DEPLOY")
REMOTE_STATUS=$?
set -e
...
printf '%s\n' "$REMOTE_OUTPUT" | while IFS= read -r line; do
  case "$line" in
    DEPLOY_STAGE=PREMUTATION_READY|DEPLOY_STAGE=BACKUP_VERIFIED|DEPLOY_STAGE=RELEASE_STAGED|DEPLOY_STAGE=MIGRATION_FIRST_PASS|DEPLOY_STAGE=MIGRATION_IDEMPOTENT|DEPLOY_STAGE=MIGRATION_VERIFIED|DEPLOY_STAGE=PROJECTS_REGISTERED|DEPLOY_STAGE=CONTROL_POINTER|DEPLOY_STAGE=WEB_RELEASE_STAGED|DEPLOY_STAGE=NGINX_CONFIGURED|DEPLOY_STAGE=SERVICE_RELOAD|DEPLOY_STAGE=M3D_REGRESSION|DEPLOY_STAGE=CONTROL_ROUTE) printf '%s\n' "$line" ;;
    DEPLOY_FAILED_AT=PREMUTATION_READY|DEPLOY_FAILED_AT=BACKUP_VERIFIED|DEPLOY_FAILED_AT=RELEASE_STAGED|DEPLOY_FAILED_AT=MIGRATION_FIRST_PASS|DEPLOY_FAILED_AT=MIGRATION_IDEMPOTENT|DEPLOY_FAILED_AT=MIGRATION_VERIFIED|DEPLOY_FAILED_AT=PROJECTS_REGISTERED|DEPLOY_FAILED_AT=CONTROL_POINTER|DEPLOY_FAILED_AT=WEB_RELEASE_STAGED|DEPLOY_FAILED_AT=NGINX_CONFIGURED|DEPLOY_FAILED_AT=SERVICE_RELOAD|DEPLOY_FAILED_AT=M3D_REGRESSION|DEPLOY_FAILED_AT=CONTROL_ROUTE|DEPLOY_FAILED_AT=REMOTE_TRANSPORT|DEPLOY_FAILED_AT=REMOTE_RESULT_MISSING) printf '%s\n' "$line" ;;
