...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "Usage: $0 [--dry-run] | --deploy --approve --owner-auth [--compat-refresh|--assistant-workstream|--workspace-continuity|--unified-workspace|--final-product|--founding-memory] [--cleanup-topology]" >&2; exit 2 ;;
  esac
done
...
FILES="hub/public/control-plane.php hub/src/HubEnrollmentService.php hub/src/HubEnrollmentApiMigration.php hub/src/HubControlPlaneService.php hub/src/HubControlPlaneRouter.php hub/src/HubBrowserOriginPolicy.php hub/src/HubControlPlaneMigration.php hub/src/HubControlPlaneProjectRegistration.php hub/src/HubOwnerAuthMigration.php hub/src/HubOwnerAuthService.php hub/src/HubOwnerAuthRouter.php hub/src/HubAssistantWorkstreamMigration.php hub/src/HubWorkspaceContinuityMigration.php hub/src/HubUnifiedWorkspaceMigration.php hub/src/HubFinalProductMigration.php hub/src/HubFoundingMemorySeed.php hub/src/HubFoundingMemoryMigration.php hub/src/HubFoundingMemoryService.php hub/src/HubAttachmentStore.php hub/src/HubNativeAgentService.php hub/migrations/002_m3e2_enrollment_api.sql hub/migrations/003_m4_control_plane.sql hub/migrations/004_owner_auth.sql hub/migrations/005_assistant_workstream.sql hub/migrations/006_workspace_continuity.sql hub/migrations/007_unified_workspace.sql hub/migrations/008_final_product.sql hub/migrations/009_founding_memory.sql hub/bin/migrate-m4.php hub/bin/migrate-owner-auth.php hub/bin/migrate-assistant-workstream.php hub/bin/migrate-workspace-continuity.php hub/bin/migrate-unified-workspace.php hub/bin/migrate-final-product.php hub/bin/migrate-founding-memory.php hub/bin/register-m4-projects.php hub/bin/setup-owner-auth.php hub/bin/verify-owner-auth-runtime.php deploy/nginx/awh-control-plane.conf deploy/nginx/render-control-plane-include.php deploy/nginx/transform-owner-auth.php deploy/awh-enrollment/insert-nginx-include.php deploy/awh-control-plane/remote-deploy-control-plane.sh dist-web/index.html dist-web/styles.css dist-web/app.js dist-web/hub-read-adapter.js dist-web/control-plane-adapter.js dist-web/manifest.webmanifest dist-web/sw.js dist-web/logo-256x256.png dist-web/web-config.json dist-web/data.json dist-web/release.json"
...
if test "$MODE" = dry-run; then
  ...
  if test "$FOUNDING_MEMORY" -eq 1; then
...
if test $((COMPAT_REFRESH + ASSISTANT_WORKSTREAM + WORKSPACE_CONTINUITY + UNIFIED_WORKSPACE + FINAL_PRODUCT + FOUNDING_MEMORY)) -gt 1; then echo "Choose one bounded activation mode" >&2; exit 2; fi
...
OWNER_PASSWORD=
if test "$ASSISTANT_WORKSTREAM" -eq 0 && test "$WORKSPACE_CONTINUITY" -eq 0 && test "$UNIFIED_WORKSPACE" -eq 0 && test "$FINAL_PRODUCT" -eq 0 && test "$FOUNDING_MEMORY" -eq 0; then
...
set +e
if test "$ASSISTANT_WORKSTREAM" -eq 1 || test "$WORKSPACE_CONTINUITY" -eq 1 || test "$UNIFIED_WORKSPACE" -eq 1 || test "$FINAL_PRODUCT" -eq 1 || test "$FOUNDING_MEMORY" -eq 1; then
  REMOTE_OUTPUT=$(ssh -o BatchMode=yes -o StrictHostKeyChecking=yes "$TARGET" sh "$REMOTE_SCRIPT" "$DB_PATH" "$REMOTE_ROOT" "$REMOTE_STAGE" "/opt/awh-hub/control-releases/$RELEASE_ID" "$RELEASE_ID" "$NGINX_CONFIG" "$HOSTNAME" "$AWH_FPM_SOCKET" "$AWH_FPM_SERVICE" "$CLEANUP_TOPOLOGY" "$OWNER_USERNAME" "$OWNER_AUTH" "$REMOTE_SCRIPT" "$COMPAT_REFRESH" "$ASSISTANT_WORKSTREAM" "$WORKSPACE_CONTINUITY" "$UNIFIED_WORKSPACE" "$FINAL_PRODUCT" "$FOUNDING_MEMORY")
else
