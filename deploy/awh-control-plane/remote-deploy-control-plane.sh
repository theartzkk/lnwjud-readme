...
verify_web_access() { sudo -n -u www-data test -x /var; sudo -n -u www-data test -x /var/www; sudo -n -u www-data test -x /var/www/awh-web; sudo -n -u www-data test -x /var/www/awh-web/releases; sudo -n -u www-data test -x "$WEB_RELEASE"; sudo -n -u www-data test -r "$WEB_RELEASE/index.html"; sudo -n -u www-data test -r "$WEB_RELEASE/awh-design-system.css"; sudo -n -u www-data test -r "$WEB_RELEASE/navigation.js"; sudo grep -q -- '--awh-font-sans' "$WEB_RELEASE/awh-design-system.css"; sudo -n -u www-data test -r "$WEB_RELEASE/database.html"; sudo -n -u www-data test -r "$WEB_RELEASE/database.css"; sudo -n -u www-data test -r "$WEB_RELEASE/database.js"; sudo grep -q '"mode": "CONTROL"' "$WEB_RELEASE/web-config.json"; sudo grep -q '"mode": "CONTROL"' "$WEB_RELEASE/data.json"; ! sudo grep -q 'Remote Preview\|Preview only\|static build' "$WEB_RELEASE/data.json"; sudo grep -q "awh-shell-$RELEASE_ID" "$WEB_RELEASE/sw.js"; }
