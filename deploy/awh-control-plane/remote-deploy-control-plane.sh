...
OWNER_PASSWORD=
if test "$ASSISTANT_WORKSTREAM" = 0 && test "$WORKSPACE_CONTINUITY" = 0 && test "$UNIFIED_WORKSPACE" = 0 && test "$FINAL_PRODUCT" = 0 && test "$FOUNDING_MEMORY" = 0 && test "$SELF_SERVICE" = 0 && test "$CENTRAL_PROJECT_AUTHORITY" = 0 && test "$ANYWHERE_EXECUTION" = 0 && test "$COST_AWARE_AI" = 0 && test "$AUTOMATIONS" = 0 && test "$SELF_SUFFICIENT_AI" = 0 && test "$ACCOUNT_HOSTING" = 0; then IFS= read -r OWNER_PASSWORD || exit 20; case "$OWNER_PASSWORD" in ''|*[!A-Za-z0-9._~-]*) exit 20 ;; esac; fi
...
if test "$ACCOUNT_HOSTING" = 1; then
...
if test "$CENTRAL_PROJECT_AUTHORITY" = 1 || test "$ANYWHERE_EXECUTION" = 1 || test "$COST_AWARE_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$SELF_SUFFICIENT_AI" = 1 || test "$ACCOUNT_HOSTING" = 1; then
  # M12 first activation may create units; all later authority refreshes require proven managed units.
  if test "$ACCOUNT_HOSTING" = 1 || test "$SELF_SUFFICIENT_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$COST_AWARE_AI" = 1 || test "$ANYWHERE_EXECUTION" = 1; then test "$EXECUTOR_UNITS_PREEXISTING" -eq 1; elif test "$M12_REFRESH" -eq 0; then test ! -e "$EXECUTOR_SERVICE_UNIT" && test ! -e "$EXECUTOR_TIMER_UNIT"; else test "$EXECUTOR_UNITS_PREEXISTING" -eq 1; fi
