...
sudo -u awh-hub env AWH_HUB_DB_PATH="$DB" /usr/bin/php "$RELEASE/hub/bin/register-m4-projects.php" >/dev/null; test "$(sudo sqlite3 "$DB" "SELECT count(*) FROM projects WHERE project_id IN ('d1e48976-cfde-479d-9a9c-f3b0ab5ec4fc','dad35312-06d6-488b-9ed2-f4886d5394ac');")" = 2; stage PROJECTS_REGISTERED
...
web_pointer_capture; sudo install -d -o awh-hub -g awh-hub -m 0750 /var/www/awh-web/releases; if sudo test -e "$WEB_RELEASE" || sudo test -L "$WEB_RELEASE"; then exit 20; fi; sudo install -d -o awh-hub -g awh-hub -m 0750 "$WEB_RELEASE"; WEB_CREATED=1; sudo cp -a "$RELEASE/dist-web/." "$WEB_RELEASE/"; sudo chown -R awh-hub:awh-hub "$WEB_RELEASE"; sudo rm -f "$WEB_POINTER_TMP"; sudo ln -s "$WEB_RELEASE" "$WEB_POINTER_TMP"; sudo mv -Tf "$WEB_POINTER_TMP" "$WEB_POINTER"; WEB_POINTER_CHANGED=1; test "$(readlink "$WEB_POINTER")" = "$WEB_RELEASE"; stage WEB_RELEASE_STAGED
CONTROL_INCLUDE=$REMOTE_ROOT/control-plane-current/deploy/nginx/awh-control-plane.conf; sudo sed "s/PREVIEW_HOSTNAME/$HOSTNAME/g" "$RELEASE/deploy/nginx/awh-control-plane.conf" > "$RELEASE/deploy/nginx/awh-control-plane.active.conf"; sudo install -o root -g root -m 0640 "$RELEASE/deploy/nginx/awh-control-plane.active.conf" "$RELEASE/deploy/nginx/awh-control-plane.conf"; sudo cp -p "$NGINX_CONFIG" "$NGINX_BACKUP"; NGINX_TMP=$(sudo mktemp /tmp/awh-control-nginx.XXXXXX); sudo /usr/bin/php "$RELEASE/deploy/awh-enrollment/insert-nginx-include.php" "$NGINX_CONFIG" "$NGINX_TMP" "$CONTROL_INCLUDE"; sudo install -o root -g root -m 0644 "$NGINX_TMP" "$NGINX_CONFIG"; sudo rm -f "$NGINX_TMP"; NGINX_CHANGED=1; stage NGINX_CONFIGURED; sudo nginx -t >/dev/null
