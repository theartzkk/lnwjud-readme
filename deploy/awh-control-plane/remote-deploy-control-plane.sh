...
if test "$CENTRAL_PROJECT_AUTHORITY" = 1 || test "$ANYWHERE_EXECUTION" = 1 || test "$COST_AWARE_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$CLOUD_FIRST" = 1; then sudo test -f "$RELEASE/hub/src/HubArtifactStore.php"; fi
if test "$ANYWHERE_EXECUTION" = 1 || test "$COST_AWARE_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$CLOUD_FIRST" = 1; then sudo test -f "$RELEASE/hub/bin/migrate-anywhere-execution.php"; sudo test -f "$RELEASE/hub/migrations/012_anywhere_execution_fabric.sql"; sudo test -f "$RELEASE/hub/src/HubAnywhereExecutionMigration.php"; sudo test -f "$RELEASE/hub/src/HubCapabilityRegistryService.php"; fi
if test "$COST_AWARE_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$CLOUD_FIRST" = 1; then sudo test -f "$RELEASE/hub/bin/migrate-cost-aware-ai.php"; sudo test -f "$RELEASE/hub/migrations/013_cost_aware_ai.sql"; sudo test -f "$RELEASE/hub/src/HubCostAwareAiMigration.php"; sudo test -f "$RELEASE/hub/src/HubProviderPricingService.php"; fi
if test "$AUTOMATIONS" = 1 || test "$SELF_SUFFICIENT_AI" = 1 || test "$CLOUD_FIRST" = 1; then sudo test -f "$RELEASE/hub/bin/migrate-automations.php"; sudo test -f "$RELEASE/hub/migrations/014_automations.sql"; sudo test -f "$RELEASE/hub/src/HubAutomationMigration.php"; sudo test -f "$RELEASE/hub/src/HubAutomationRegistryService.php"; sudo test -f "$RELEASE/hub/src/HubAutomationSchedulerService.php"; fi
if test "$CLOUD_FIRST" = 1; then sudo test -f "$RELEASE/hub/bin/migrate-cloud-first.php"; sudo test -f "$RELEASE/hub/migrations/017_cloud_first_control.sql"; sudo test -f "$RELEASE/hub/src/HubCloudFirstMigration.php"; sudo test -f "$RELEASE/hub/src/HubCloudWorkflowService.php"; sudo test -s "$RELEASE/.awh-build/awh-source.zip"; fi
if test "$ACCOUNT_HOSTING" = 1; then sudo test -f "$RELEASE/hub/bin/migrate-account-hosting.php"; sudo test -f "$RELEASE/hub/migrations/016_account_hosting.sql"; sudo test -f "$RELEASE/hub/src/HubAccountHostingMigration.php"; sudo test -f "$RELEASE/hub/src/HubManagedHostingService.php"; sudo test -f "$RELEASE/hub/src/HubManagedHostingOperator.php"; sudo test -f "$RELEASE/hub/bin/awh-hosting-operator.php"; sudo test -f "$RELEASE/deploy/systemd/awh-hosting-operator.service"; sudo test -f "$RELEASE/deploy/systemd/awh-hosting-operator.timer"; sudo test -s "$RELEASE/.awh-build/awh-source.zip"; fi
...
if test "$CENTRAL_PROJECT_AUTHORITY" = 1 || test "$ANYWHERE_EXECUTION" = 1 || test "$COST_AWARE_AI" = 1 || test "$AUTOMATIONS" = 1 || test "$SELF_SUFFICIENT_AI" = 1 || test "$ACCOUNT_HOSTING" = 1 || test "$CLOUD_FIRST" = 1; then
...
if test "$CLOUD_FIRST" = 1; then
  ...
  sudo install -d -o root -g root -m 0755 /etc/nginx/sites-available /etc/nginx/sites-enabled
...
if test "$CLOUD_FIRST" = 1; then stage CLOUD_FIRST_ROUTE; test "$(sudo sqlite3 "$DB" "SELECT count(*) FROM control_capability_catalog WHERE capability IN ('qa.cloud','review.visual') AND enabled=1;")" = 2; fi
