...
stage OWNER_AUTH_WEB_SURFACE; root_code=$(curl --silent --max-time 10 --resolve "$HOSTNAME:443:127.0.0.1" -o /dev/null -w '%{http_code}' "https://$HOSTNAME/" 2>/dev/null || printf 000); test "$root_code" = 200; database_html_code=$(curl --silent --max-time 10 --resolve "$HOSTNAME:443:127.0.0.1" -o /dev/null -w '%{http_code}' "https://$HOSTNAME/database.html" 2>/dev/null || printf 000); test "$database_html_code" = 200; database_api_code=$(curl --silent --max-time 10 --resolve "$HOSTNAME:443:127.0.0.1" -H 'Sec-Fetch-Site: same-origin' -o /dev/null -w '%{http_code}' "https://$HOSTNAME/database-studio.php?action=overview" 2>/dev/null || printf 000); test "$database_api_code" = 401; control_config_code=$(curl --silent --max-time 10 --resolve "$HOSTNAME:443:127.0.0.1" -o /tmp/awh-control-web-config-$RELEASE_ID.json -w '%{http_code}' "https://$HOSTNAME/web-config.json" 2>/dev/null || printf 000); test "$control_config_code" = 200; grep -q '"mode": "CONTROL"' /tmp/awh-control-web-config-$RELEASE_ID.json; sudo rm -f /tmp/awh-control-web-config-$RELEASE_ID.json; preview_code=$(curl --silent --max-time 10 --resolve "$HOSTNAME:443:127.0.0.1" -o /dev/null -w '%{http_code}' "https://$HOSTNAME/preview/" 2>/dev/null || printf 000); test "$preview_code" = 401
