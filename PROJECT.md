...
- M3E.1 additive SQLite migration, M3E.2 enrollment API, and M4 control-plane migration are deployed on the verified ReadyIDC production Hub; the database is at schema version 4 with the M3D project and memory metadata preserved.
...
- Owner access v1 is implemented locally as an additive v4→v5 capability: username/password authentication attaches to the existing owner identity and canonical control sessions, with secure password hashes, throttling, CSRF, remember-session expiry, logout/revocation, password change, one-time hashed recovery codes, step-up verification and sanitized audit events. ReadyIDC was not mutated by this source pass; optional Passkey remains deferred.
...
- ReadyIDC M4 production remains the active control-plane authority at the previously verified v4 baseline; this source pass does not activate owner-auth v5. Existing M3D/M3E data and the enrolled Mac remain outside this source-only change.
...
- The normal browser owner path is username/password. The pairing-code entry remains only as a compatibility option until the v5 auth deployment is approved. Basic Auth rotation is not part of this pass.
