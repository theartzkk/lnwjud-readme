...
- M4 production activation has not occurred. ReadyIDC remains the active M3D/M3E production authority with DB v3, clean integrity/FK state, one indexed project, and one enrolled Mac; the additive M4 schema/API/PWA CONTROL release and executable rollback package are prepared locally, activate with zero required user projects, and require one bounded production approval. Projects can be added later through the reusable onboarding path.
