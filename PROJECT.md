...
- M4 local control-plane foundation is implemented as one additive package: canonical Hub tasks/events, HTTP-only mobile control sessions, CSRF/origin protection, project authorization, idempotent Goal submission, truthful WAITING_FOR_WORKER state, bounded worker heartbeat/lease/claim/update, artifact/approval metadata tables, and a native Desktop worker client that reuses the enrolled device credential. It is not activated on ReadyIDC.
- The web build has an explicit CONTROL mode (`npm run web:build:control`) for the same canonical project/task model; the ordinary web build remains the safe static preview. Local fixtures prove phone session → project → Goal → queue → worker claim → bounded completion, but no live iPhone task submission is claimed before the one M4 activation approval.
...
- M4 production activation has not occurred. ReadyIDC remains the active M3D/M3E production authority with DB v3, clean integrity/FK state, one indexed project, and one enrolled Mac; the additive M4 schema/API/static CONTROL release and rollback package are prepared locally and require one bounded production approval.
...
- iPhone control is locally prepared but not field-live: the first browser trust uses one owner-issued pairing code, then an HttpOnly Secure session cookie; permanent native device credentials are never placed in browser storage. Without an online eligible worker, the canonical task remains WAITING_FOR_WORKER.
