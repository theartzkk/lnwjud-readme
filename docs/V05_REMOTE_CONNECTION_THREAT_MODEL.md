# AWH v0.5 — Remote Connection Threat Model

## Decision

AWH is designed for OpenAI Secure MCP Tunnel private remote connectivity. It will **not** expose a public HTTP MCP listener or open inbound firewall ports. Actual OpenAI control-plane end-to-end connectivity is not verified by the local tests in this repository.

The existing `serveStdio()` MCP factory remains the Source of Truth. The tunnel is transport only, and the MCP protocol identity remains the legacy `art-agent` compatibility identity for now.
...
AWH therefore uses the packaged Electron binary as the self-contained Node runtime for the already-built `dist/index.js`. The MCP server still comes from `src/stdio.ts` / `createServer()`; only the process mode changes.
...
- Admin credentials for tunnel CRUD are outside AWH's long-lived runtime path.
...
- AWH may manage only a `tunnel-client` process that it launched in the current runtime.
- Stop/restart operations target only that owned process tree.
- The MCP command and Electron Node-mode environment are constructed internally from the installed AWH Desktop path and selected canonical workspace.
...
- AWH does not weaken the Electron renderer CSP or add generic HTTP fetch capability.
