...
test('canonical Dashboard is intent-first with a four-item mobile navigation',async()=>{
 ...
 for(const label of ['หน้าแรก','แชท','งาน','ไฟล์']) assert.match(dashboard,new RegExp(label));
 for(const leaked of ['งาน/AI','Cloud พร้อมใช้งาน','ทุกงาน เริ่มจากตรงนี้']) assert.doesNotMatch(dashboard,new RegExp(leaked));
 assert.match(css,/awh-mobile-nav/); assert.match(css,/repeat\(4,minmax\(0,1fr\)\)/);
