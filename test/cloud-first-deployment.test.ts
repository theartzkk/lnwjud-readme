...
test('M18 Cloud-first activation contract is additive, rollback-safe and approval-gated',async()=>{
 ...
 assert.match(remoteText,/M18_TABLE_COUNT_BEFORE/); assert.match(remoteText,/class_exists\(\"ZipArchive\"\)/); assert.match(remoteText,/provider-credentials/); assert.match(remoteText,/m18-cloud-first-control/); assert.match(remoteText,/CLOUD_FIRST_ROUTE/); assert.match(remoteText,/PROJECT_SOURCE_AUTHORITY\" = 1 \|\| test \"\$IDENTITY_CONVERGENCE\" = 1; then DB_MUTATED=1; stage PROJECT_VAULT_SOURCE_SYNC/);
