...
test('M17 Account + Managed Hosting activation is additive, typed and approval-gated',async()=>{
 ...
 assert.match(ownerCenter,/URLSearchParams\(window\.location\.search\)/); assert.match(ownerCenter,/window\.location\.hash === '#source'/); assert.match(hostingHtml,/id="site-url-preview"/); assert.match(hostingHtml,/id="hosting-confirm-submit"/); assert.match(hostingHtml,/id="hosting-ecosystem"/); assert.match(hostingJs,/renderUrlPreview/); assert.match(hostingJs,/renderEcosystem/); assert.match(hostingJs,/site\.lastEvent/); assert.match(hostingJs,/sourceInstruction/); assert.match(hostingJs,/owner-center\.html\?project=/); assert.match(hostingJs,/Source พร้อม/); assert.match(hostingJs,/bindManagedSiteDomain/); assert.match(hostingJs,/needsLiveRefresh/); assert.match(hostingJs,/refreshHostingData/); assert.match(hostingJs,/visibilitychange/); assert.match(hostingJs,/clearTimeout\(liveRefreshTimer\)/); assert.match(hostingJs,/กำลังรอ DNS|รอ DNS/); assert.match(hostingJs,/confirmAction/); assert.doesNotMatch(hostingJs,/requestStepUp|stepUp\(|hosting-stepup-password/); assert.match(trustPolicy,/hosting\.site\.create/); assert.match(trustPolicy,/hosting\.site\.deploy/); assert.match(trustPolicy,/account\.user\.access/);
});
