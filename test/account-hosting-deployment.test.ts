...
test('M17 Account + Managed Hosting activation is additive, typed and approval-gated',async()=>{
 ...
 assert.ok(refreshStart>=0&&refreshEnd>refreshStart); assert.match(remoteText.slice(Math.max(0,refreshStart-700),refreshEnd),/HOSTING_RUNTIME_READY/); assert.match(refreshHosting,/install -o root -g root -m 0644 "\$RELEASE\/deploy\/systemd\/awh-hosting-operator\.timer" "\$HOSTING_TIMER_UNIT"/); assert.match(refreshHosting,/HOSTING_UNITS_INSTALLED=1/); assert.match(refreshHosting,/systemctl daemon-reload/);
 ...
 assert.match(timer,/OnActiveSec=5s/); assert.match(operator,/hosting\.site\.bind_domain/); assert.match(operator,/DOMAIN_DNS_NOT_READY/); assert.match(operator,/DOMAIN_ROUTE_CONFLICT/); assert.match(operator,/certbot/); assert.match(operator,/--deploy-hook/); assert.match(operator,/renderDomainHttps/); assert.match(operator,/UPDATE control_managed_sites SET public_mode='DOMAIN'/); assert.match(operator,/awh-domain-/); assert.match(timer,/OnUnitActiveSec=30s/); assert.match(operator,/does match certificate/); assert.match(operator,/letsencrypt\/live/); assert.match(build,/asset\('hosting\.html'\)/); assert.match(verifier,/'hosting\.html'/);
 assert.match(ownerCenter,/URLSearchParams\(window\.location\.search\)/); assert.match(ownerCenter,/window\.location\.hash === '#source'/); assert.match(hostingHtml,/id="site-url-preview"/); assert.match(hostingHtml,/id="hosting-confirm-submit"/); assert.match(hostingHtml,/id="hosting-ecosystem"/); assert.match(hostingHtml,/กรอกเพียง 3 อย่าง/); assert.match(hostingHtml,/รายละเอียดระบบ/); assert.doesNotMatch(hostingHtml,/OWNER · WEB|QUICK CREATE|MY WEBSITES|Hosting Operator|Internal port|Production|Rollback|Deploy/); assert.match(hostingJs,/renderUrlPreview/); assert.match(hostingJs,/site\.source\?\.ready===true/); assert.match(hostingJs,/renderEcosystem/); assert.match(hostingJs,/site\.lastEvent/); assert.match(hostingJs,/sourceInstruction/); assert.match(hostingJs,/owner-center\.html\?project=/); assert.match(hostingJs,/ไฟล์ต้นทาง: พร้อม/); assert.match(hostingJs,/friendlyError/); assert.match(hostingJs,/กำลังเชื่อมชื่อเว็บ/); assert.match(hostingJs,/bindManagedSiteDomain/); assert.match(hostingJs,/needsLiveRefresh/); assert.match(hostingJs,/refreshHostingData/); assert.match(hostingJs,/visibilitychange/); assert.match(hostingJs,/clearTimeout\(liveRefreshTimer\)/); assert.match(hostingJs,/confirmAction/); assert.doesNotMatch(hostingJs,/requestStepUp|stepUp\(|hosting-stepup-password/); assert.match(trustPolicy,/hosting\.site\.create/); assert.match(trustPolicy,/hosting\.site\.deploy/); assert.match(trustPolicy,/account\.user\.access/);
});
