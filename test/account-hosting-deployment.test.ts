...
test('M17 Account + Managed Hosting activation is additive, typed and approval-gated',async()=>{
 ...
 assert.match(timer,/OnActiveSec=5s/); assert.match(operator,/hosting\.site\.bind_domain/); assert.match(operator,/DOMAIN_DNS_NOT_READY/); assert.match(operator,/certbot/); assert.match(operator,/renderDomainHttps/); assert.match(timer,/OnUnitActiveSec=30s/); assert.match(operator,/does match certificate/); assert.match(operator,/letsencrypt\/live/); assert.match(build,/asset\('hosting\.html'\)/); assert.match(verifier,/'hosting\.html'/);
 assert.match(hostingHtml,/id="site-url-preview"/); assert.match(hostingHtml,/id="hosting-confirm-submit"/); assert.match(hostingJs,/renderUrlPreview/); assert.match(hostingJs,/bindManagedSiteDomain/); assert.match(hostingJs,/กำลังรอ DNS|รอ DNS/); assert.match(hostingJs,/confirmAction/); assert.doesNotMatch(hostingJs,/requestStepUp|stepUp\(|hosting-stepup-password/); assert.match(trustPolicy,/hosting\.site\.create/); assert.match(trustPolicy,/hosting\.site\.deploy/); assert.match(trustPolicy,/account\.user\.access/);
});
