...
test('M17 Account + Managed Hosting activation is additive, typed and approval-gated',async()=>{
 ...
 const refreshStart=remoteText.indexOf('if test "$IDENTITY_CONVERGENCE" = 1 || test "$CONVERSATION_LIFECYCLE" = 1 || test "$PROJECT_SOURCE_AUTHORITY" = 1 || test "$CLOUD_FIRST" = 1; then');
 ...
 assert.match(service,/^\[Unit\]/m); assert.doesNotMatch(service,/AWH_PUBLIC_HOST=157\.85\.108\.142/); assert.match(service,/ExecStart=\/usr\/bin\/php .*awh-hosting-operator\.php/); assert.doesNotMatch(service,/sh -c|bash -c/); assert.match(service,/ReadWritePaths=.*\/etc\/letsencrypt/); assert.match(service,/ReadWritePaths=.*\/var\/lib\/awh-acme/); assert.match(service,/ReadWritePaths=.*\/etc\/subuid .*\/etc\/subgid/); assert.match(service,/ReadWritePaths=.*-\/var\/backups\/learnlab-releases/); assert.match(service,/ReadWritePaths=.*-\/var\/backups\/bay-assessment/); assert.match(service,/ReadOnlyPaths=-\/var\/lib\/awh-remote\/handoff -\/home\/bayadmin\/\.awh\/source\/git\/bay-assessment\.git/);
