...
test('M17 Account + Managed Hosting activation is additive, typed and approval-gated',async()=>{
 ...
 assert.match(hostingHtml,/id="site-url-preview"/); assert.match(hostingHtml,/id="hosting-stepup-password"/); assert.match(hostingJs,/renderUrlPreview/); assert.match(hostingJs,/requestStepUp/);
});
...
test('browser keeps typed step-up code private while exposing a human hosting message',async()=>{
 ...
 await assert.rejects(controlRequest('/api/v1/control/hosting/sites',{method:'POST',body:'{}'},fetchImpl),error=>error instanceof Error && error.message==='กรุณายืนยันรหัสผ่านก่อนบันทึกการตั้งค่าสำคัญ' && (error as Error & {code?:string}).code==='STEP_UP_REQUIRED' && !error.message.includes('STEP_UP_REQUIRED'));
});
