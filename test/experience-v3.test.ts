...
test('canonical Work markup and renderer own the mobile chat experience',async()=>{
 ...
 assert.match(app,/renderArtifactCard/); assert.match(app,/localProgressLabel/); assert.match(html,/class="send-label"/); assert.match(css,/result-file-card/); assert.match(css,/send-label \{ display: none/);
