...
test('canonical Work markup and renderer own the mobile chat experience',async()=>{
 ...
 assert.match(app,/renderArtifactCard/); assert.match(app,/renderLiveActivity/); assert.match(app,/announceTaskMilestones/); assert.match(html,/id="work-announcer"[^>]*aria-live="polite"/); assert.match(css,/awh-live-pulse/); assert.match(app,/localProgressLabel/); assert.match(html,/class="send-label"/); assert.match(css,/result-file-card/); assert.match(css,/send-label \{ display: none/);
 assert.match(html,/id="goal-stop"/); assert.match(app,/activeCancellableTask/); assert.match(app,/addPendingAttachments/); assert.match(app,/clipboardData\?\.files/); assert.match(app,/dataTransfer\?\.files/); assert.match(app,/copyMessageText/); assert.match(css,/composer\.is-drop-target/);
