...
test('Trust Center ships as a canonical mobile-safe release surface without browser authority',async()=>{
 ...
 try{
  ...
  const [html,css,js,manifest,deploy]=await Promise.all(['trust.html','trust.css','trust.js'].map(f=>readFile(join(output,f),'utf8')).concat([readFile(join(ROOT,'scripts/web-release-files.json'),'utf8'),readFile(join(ROOT,'deploy/awh-control-plane/deploy-control-plane.sh'),'utf8')]) as Promise<string[]>);
  assert.match(html,/Trust Center/);assert.match(html,/TRUST & CONTROL/);assert.match(css,/@media\(max-width:760px\)/);assert.match(css,/--accent:var\(--awh-accent\)/);assert.match(js,/\/api\/v1\/control\/trust/);
