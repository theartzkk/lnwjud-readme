...
test('Infrastructure is an Owner-only sanitized projection and canonical web surface',async()=>{
 ...
 try{
  ...
  for(const projection of ['productionComplete','aiModels','autonomousWork','incidents','morningBrief','stagedCandidates']) assert.match(js,new RegExp(projection));
  ...
  assert.match(executor,/telemetryRefreshIfStale\(null, 60\)/); assert.match(executor,/'telemetry' => \$telemetry/);
