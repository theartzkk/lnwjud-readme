...
test('Infrastructure is an Owner-only sanitized projection and canonical web surface',async()=>{
 ...
 try{
  ...
  for(const projection of ['productionComplete','aiModels','autonomousWork','incidents','stagedCandidates','staff','governor','selfHealing','executionTriage','CURRENT_DEFECT','SETUP_REQUIRED','POLICY_PAUSED','BLOCKED_CAPABILITY','morningBrief','storageGovernance','managedSites','hostingCenter','UNKNOWN']) assert.match(js,new RegExp(projection));
