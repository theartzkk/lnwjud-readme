...
test('M17 Database Studio web release is owner-first, read-only, and emitted by the canonical web build', async () => {
  ...
  try {
    ...
    const [html, css, app] = await Promise.all([readFile(join(output, 'database.html'), 'utf8'), readFile(join(output, 'database.css'), 'utf8'), readFile(join(output, 'database.js'), 'utf8')]);
