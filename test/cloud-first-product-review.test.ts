...
test('Cloud-first Product Review is emitted by canonical build and deploy bundle', async()=>{
  ...
  try {
    ...
    assert.match(js,/loadCloudRevision/); assert.match(js,/submitCloudTask/); assert.match(js,/cancelTask/); assert.match(js,/stepUp/); assert.match(js,/updateCloudCredential/); assert.match(js,/cloudReviewEligible/); assert.match(js,/ป้องกัน Source ผิดโปรเจกต์/); assert.match(js,/review-project'\)\?\.addEventListener\('change'/);
    assert.doesNotMatch(`${html}\n${js}`,/localStorage|sessionStorage|Authorization|Bearer\s/);
    assert.match(controlService,/str_starts_with\(\(string\)\(\$project\['provenance'\]\?\?''\),'release-vault:'\)/); assert.match(controlService,/CLOUD_PROJECT_SOURCE_MISMATCH/); assert.match(controlService,/cloudReviewEligible/);
