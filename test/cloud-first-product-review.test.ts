...
test('Cloud-first Product Review is emitted by canonical build and deploy bundle', async()=>{
  ...
  try {
    ...
    assert.match(index,/id="session-check-view"/); assert.match(index,/id="sign-in-view"[\s\S]*?hidden/); assert.match(owner,/commitAwhSurface/); assert.match(owner,/onAwhSurfaceChange/); assert.match(owner,/awh-surface/);
    assert.match(js,/loadProjectSourceAuthority/); assert.match(js,/submitCloudTask/); assert.match(js,/cancelTask/); assert.match(js,/stepUp/); assert.match(js,/updateCloudCredential/);
