...
- Canonical branch authority: `awh/api-independence`; exact SHA must be resolved from `origin` at read time. Last verified canonical source baseline: `54ec9570460102bc5eee13239214adecde698de7` (resolve from origin again before every mutation).
...
- Control/Web pointers: `m20-54ec95704601`.
...
- M20 Production activation completed through the canonical `--deploy --approve --owner-auth --project-source-authority` path. The latest schema20 source refresh is release `m20-54ec95704601`; it did not re-run migration 019 and installed the versioned hosting operator units from the exact release.
- AWH Project Vault has an ACTIVE post-refresh source archive revision (`8e4bd559-dc44-4e5a-a54b-553fd77b1b94`) containing 498 tracked files; the prior post-activation revision is superseded.
...
- Fresh post-refresh backup `awh-20260902T081712Z.sqlite` is verified at 65,134,592 bytes, SHA-256 `9d3842e2c30db8d1b68577667e2d6bf5c196576e0d912fd4197746a87dc93ee4`, schema 20, integrity `ok`, with no foreign-key violation output.
