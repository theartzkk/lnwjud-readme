...
- M3E-FINAL tests cover native adapter argv/stdin isolation, missing/malformed OS credential records, platform selection, self-revoke, Desktop enrollment IPC, M3E.2 additive migration, first-owner-to-first-device bootstrap, one-nonce bootstrap orchestration, compiled helper runtime, digest-only provisioning, two-server Nginx insertion, deployment isolation, rollback metadata/order, and secret-free templates.
