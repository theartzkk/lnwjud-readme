...
- The bounded source repair is committed in `72d06691d96b1139a001021c5cf88e6afdda7378`: `HubProjectVault` adds a provider-facing 24 KiB valid-UTF-8 read contract while preserving the internal 256 KiB read contract; `HubDurableExecutionService` uses it for all provider-facing Vault/workspace reads; `project-vault-search.php` covers large, escaped, UTF-8 content and the JSON result boundary. Candidate `61bbece02b6e54ca410cb68fa7d1cd403e902888` is a clean descendant of current canonical `e0fa5cc…`; Production remains at `06a7277…`.
- Verification: Project Vault, M16, Continuous Auto-Chain and M12 focused fixtures passed; typecheck passed; serial `npm test` passed `319/0/1`; serial `qa:full` passed; `git diff --check` passed; PR #58 CI run `33266105132` passed all 5 jobs; M16 dry-run and read-only Production preflight passed. The earlier parallel QA invocation exposed only a generated `dist-web` race and was superseded by serial evidence.

## Next action

Request one fresh exact-SHA Production approval for `61bbece02b6e54ca410cb68fa7d1cd403e902888`. Do not deploy the candidate before that approval; after approval rerun the bounded field proof and continue UAT/Production completion. Preserve `06a7277…` and the existing rollback assets until the new release passes public verification.
