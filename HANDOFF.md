...
- Current Mac-validated closure candidate `3cf66fdf0b3e048ceb922362b543b4bae5156718` passed monorepo typecheck, synchronized 214-tool catalog, Hub regression 17 PASS / 1 platform SKIP, M13 PASS, CONTROL web build/release manifest, packaging 5 PASS / 2 Windows-only SKIP, release gate 6/6 and `git diff --check`. One final Windows execution of root `package:windows` is still required to prove the newly automated path and capture installer SHA-256/install-launch smoke evidence.
- Installer UX source audit then found and fixed a real defect: the custom Start Menu shortcut still targeted nonexistent `lnwjud.exe`, and uninstall cleanup could delete legacy `lnwjud` data. It now uses electron-builder `${PRODUCT_FILENAME}` / `${APP_EXECUTABLE_FILENAME}` macros and deletes only canonical AWH data when the user explicitly chooses removal. Mac packaging contract PASS; Windows installer rerun remains the final proof.
- Packaging follow-up also selects the canonical `Art’s Workspace Hub.exe` explicitly (the unpacked root legitimately also contains private `lnwjud-node.exe`) and adds root `verify:windows-package` for VersionInfo/hash/launch smoke, with optional guarded `-InstallSmoke` install→launch→uninstall verification.
- Development branch publication is now verified: `origin/awh/lnwjud4-port` was first confirmed at `3cf66fdf0b3e048ceb922362b543b4bae5156718`. The successful transport used conservative HTTP/1.1 settings after dry-run/ref negotiation passed; no merge or deploy was performed.
